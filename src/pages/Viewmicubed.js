...
import AddUserModal from './Addmicubed';
import AddUserModalBulk from './Addmicubedbulk';
...
function ViewPage() {
  ...
  const handleImageUpload = (event) => {
    ...
    const admid1=global1.admid;
    const icucode1=global1.icucode;
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteicubedbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    //alert(row._id);
     const response = await ep1.get('/api/v2/createpbillingbyfac', {
            params: {
                ...
                colid: colid,
                name: name,
               patient: global1.patient,
    puser:global1.puser,
    admid:global1.admid,
    billdate:new Date(),
    description:'ICU bed assigned ' + row.bedno + ' type ' + row.bedtype + ' for ICU ' + global1.icucode,
    amount:row.amount,
    payref:'NA',
    paystatus:'Due',
    ...
    status1:'Submitted',
         ...
         const response1 =await ep1.get('/api/v2/updateicubedbyfac', {
            params: {
            id: row._id,
            user: user,
            token:token,
...
type:'Occupied',
            ...
            const response2 = await ep1.get('/api/v2/createpadmhistorybyfac', {
                    params: {
                        user: user,
                        token: token,
                        colid: colid,
            ...
            puser:global1.puser,
            admid:global1.admid,
            activity:'ICU bed assigned ' + row.bedno + ' type ' + row.bedtype + ' for ICU ' + global1.icucode,
            bedno:row.bedno,
            bedtype:row.bedtype,
            activitydate:new Date(),
            doccomments:'ICU bed assigned',
        ...
        alert('Bed assigned and billing created');
        fetchViewPage();
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/geticubedbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/geticubedcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/geticubedsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const bedtype=user.bedtype;
const amount=user.amount;
const patient=user.patient;
const puser=user.puser;
const admid=user.admid;
const type=user.type;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updateicubedbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
    ...
    return (
      <React.Fragment>
        <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
          Patient {patient1} , User: {puser1} , Admission ID: {admid1} , ICU Code: {icucode1}
          <br /><br />
