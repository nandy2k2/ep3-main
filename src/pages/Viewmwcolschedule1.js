...
import AddUserModal from './Addmwcolschedule1';
import AddUserModalBulk from './Addmwcolschedule1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletewcolschedule1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getwcolschedule1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getwcolschedule1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getwcolschedule1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const cday=user.cday;
const ctime=user.ctime;
...
const amount=user.amount;
            ...
            const response =await ep1.get('/api/v2/updatewcolschedule1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
