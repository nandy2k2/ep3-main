...
return;
  ...
  const searchapi = async (rownumber,editionid,author,institution,article,description,abstractlink,articlelink,pubstatus,authortype,submitdate,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Edition id' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Author' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Institution' + ',';
return;
}
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Article' + ',';
return;
}
if(!row[4]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Abstract link' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Article link' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Publication status' + ',';
return;
}
if(!row[8]){
f1=f1 + 'row ' + rownumber + '-' + 'Author type' + ',';
return;
}
 if(isNaN(new Date(row[9]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Submit date' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      editionid,author,institution,article,description,abstractlink,articlelink,pubstatus,authortype,submitdate,status <br /><br />
