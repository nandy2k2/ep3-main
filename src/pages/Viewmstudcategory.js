...
// }
    ...
    const fetchViewPage = async () => {
      ...
      const response = await ep1.get('/api/v1/getstudentbycatnew', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const handleExport = () => {
      ...
      for (var i=0; i<rows.length; i++) {
        Comment.push({
          ...
          student: rows[i]._id.admissionyear,
          regno: rows[i]._id.category,
      ...
      saveAs(data, 'Student_by_category.xlsx');
      setOpenExport(false);
    };
