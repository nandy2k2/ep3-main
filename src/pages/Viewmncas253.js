...
import AddUserModal from './Addmncas253';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v1/deletencas253byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'type',
headerName:'Type of Awards or Fellowship',
type:'dropdown',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'awards',
headerName:'Awards or Fellowship name',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v1/getncas253byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v1/getncas253countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v1/getncas253secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const score=user.score;
            ...
            const response =await ep1.get('/api/v1/updatencas253byfac', {
            params: {
            id: user._id,
            user: user,
            token:token,
