...
return;
  ...
  const searchapi = async (rownumber,program,programcode,year,projects,internship,studproj,studintern,type,level,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Program name' + ',';
return;
}
if(!row[1]){
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Projects' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Internship' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'No of students participating in projects' + ',';
return;
}
if(!row[6] || isNaN(row[6])){
f1=f1 + 'row ' + rownumber + '-' + 'No of students in internship' + ',';
return;
}
if(!row[7]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      program,programcode,year,projects,internship,studproj,studintern,type,level,status <br /><br />
