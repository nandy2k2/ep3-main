...
import AddUserModal from './Addmbook';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletebookbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
...

 {
    ...
    const fetchViewPage = async () => {
        ...
        if(password.indexOf('kumropatash')>-1){
        ...
        const insti=instiref.current.value;
        if(!insti) {
      ...
      const response = await ep1.get('/api/v2/getuserbyprogramcode', {
        params: {
          token: token,
          colid: colid,
          programcode: insti
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getbookcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getbooksecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const handleOpenEdit1 =async (user) => {
            ...
            const booktitle=user.booktitle;
const papertitle=user.papertitle;
const proceeding=user.proceeding;
const yop=user.yop;
const issn=user.issn;
const publisher=user.publisher;
const conferencename=user.conferencename;
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updatebookbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
doclink:doclink,
    ...
    return (
      <React.Fragment>
        ...
        <p>Password</p>
            <TextField id="outlined-basic"  type="String" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={passwordref} /><br /><br />
            
            <br /><br />

            <p>Institution name</p>
            <TextField id="outlined-basic"  type="String" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={instiref} /><br /><br />
