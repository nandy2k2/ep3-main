...
return;
  ...
  const searchapi = async (rownumber,year,course,coursecode,announcement,submitdate,type,target,status1) => {
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Announcement' + ',';
return;
}
 if(isNaN(new Date(row[4]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Announcement date' + ',';
return;
}
if(!row[5]){
...
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'target audience' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,course,coursecode,announcement,submitdate,type,target,status <br /><br />
