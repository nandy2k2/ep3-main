...
function ViewPage() {
  ...
  const handleImageUpload = (event) => {
    ...
    const onButtonClickq = async(e, row) => {
      ...
      const date12=new Date(row.starttime);
      ...
      const date1=new Date(row.endtime);
      const date2=new Date();
      if(date2>date1){
        alert('Exam expired');
        return;
      }
      global1.endtime=row.endtime;
      
      navigate('/dashmtestqstud');
    };
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gettestnewbyccode', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gettestnewcountbyccode', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gettestnewsecondbyccode', {
        params: {
          token: token,
          colid: colid,
          user: user,
