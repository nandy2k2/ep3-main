...
function ItemsEditor({ items, setItems, showMake = false, showPrice = false }) {
  ...
  return (
    <Paper sx={{ p: 2 }}>
      <Grid container spacing={2} alignItems="center">
        <Grid item xs={12} md={showMake || showPrice ? 4 : 7}>
    ...
    <Page title="Indent Request" subtitle="Request items from stores assigned to the logged-in user." message={message} error={error || basicsError}>
      <Paper sx={{ p: 2, mb: 2 }}>
        <Grid container spacing={2}>
          <Grid item xs={12} md={3}><Autocomplete options={creatorDepartments} getOptionLabel={(o) => o.departmentname || ""} onChange={(_, v) => setForm((p) => ({ ...p, departmentname: v?.departmentname || "" }))} renderInput={(params) => <TextField {...params} label="Department" size="small" helperText={!creatorDepartments.length ? "No department mapped to your creator email" : ""} />} /></Grid>
      ...
      <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: "wrap" }}><Button startIcon={<Save />} variant="contained" onClick={() => save("Draft")}>Save draft</Button><Button variant="contained" color="success" onClick={() => save("Submitted")}>Submit</Button><Button startIcon={<Print />} variant="outlined" disabled={!printData} onClick={() => printPurchase2("indent", printData)}>Print indent</Button></Stack>
    </Page>
  );
}
...
export function Purchase2StoreRequestReviewPage() {
  ...
  const issue = async () => {
    try {
      ...
      for (const item of items) {
        ...
        const requestedIssue = item.approvedquantity === "" || item.approvedquantity === undefined ? Math.min(requested, availableQty) : num(item.approvedquantity);
        ...
        if (item._id) await updateRow("storerequisitionitemsds2", item, { approvedquantity: issueQty, issuedquantity: issueQty, assignedquantity: issueQty, status: issueQty < requested ? "Partially Issued" : "Issued" });
      }
      if (!issuedAny) throw new Error("No quantity available to issue");
      const finalStatus = issuedLess ? "Partially Issued" : "Issued";
      const header = await updateRow("storerequisitionds2", selected, { reqstatus: finalStatus });
      setMessage("Items issued and stock updated");
      setPrintData({ header: { ...selected, ...header }, items: items.map((item) => ({ ...item, issuedquantity: item.approvedquantity || item.issuedquantity })) });
      await load();
  ...
  return (
    <Page title="Store Request Review and Issue" subtitle="Review requests for assigned stores and issue available items." message={message} error={error || basicsError}>
      ...
      {selected && <Paper sx={{ p: 2, mt: 2 }}><Typography variant="h6">Issue items</Typography><DataGrid autoHeight rows={items.map((item, i) => ({ ...item, id: i, available: stock.find((s) => text(s.storeid) === text(selected.storeid) && text(s.itemcode) === text(item.itemcode))?.quantity || 0, assignedquantity: item.assignedquantity || item.issuedquantity || item.approvedquantity || 0 }))} columns={[{ field: "itemname", headerName: "Item", flex: 1 }, { field: "quantity", headerName: "Requested", width: 120 }, { field: "available", headerName: "Available", width: 120 }, { field: "assignedquantity", headerName: "Assigned", width: 120 }, { field: "approvedquantity", headerName: "Issue qty", width: 130, editable: true }]} processRowUpdate={(row) => { setItems(items.map((it, idx) => idx === row.id ? { ...it, approvedquantity: num(row.approvedquantity) } : it)); return row; }} /><Button sx={{ mt: 2 }} variant="contained" onClick={issue}>Issue selected quantities</Button></Paper>}
      <Stack direction="row" spacing={1} sx={{ mt: 2 }}><Button startIcon={<Print />} variant="outlined" disabled={!printData} onClick={() => printPurchase2("indent", printData)}>Print issue document</Button></Stack>
    </Page>
  );
}
...
export function Purchase2LocalPoPage() {
  ...
  const createPo = async () => {
    try {
      ...
      const header = await saveRow("storepoorderds2", { poid, year: "2026-27", vendor: vendor.vendorname, vendorid: vendor._id, price: total, netprice: total, actualAmount: total, poType: "Local", postatus: "Draft", approvalStatus: "Draft", deliveryType: "Physical Delivery", terms, creatorName: currentName(), creatorEmail: currentUser(), creatorSignature: signature.signaturelink || "", description: "Local PO" });
...
function Purchase2ManagePoPage({ admin = false }) {
  ...
  return <Page title={admin ? "Manage PO Admin" : "Manage PO Creator"} subtitle={admin ? "View all PR and PO records and approve submitted POs." : "Create POs from PRs assigned to you and submit them for approval."} message={message} error={error}><Paper sx={{ p: 2, mb: 2 }}><Typography variant="h6">PR list</Typography><DataGrid autoHeight rows={(admin ? prs : assignments).map((r) => ({ ...r, id: r._id }))} columns={admin ? [{ field: "prnumber", headerName: "PR", width: 160 }, { field: "storename", headerName: "Store", width: 170 }, { field: "requestedbyemail", headerName: "Requested By", width: 220 }, { field: "status", headerName: "Status", width: 120 }] : [{ field: "prnumber", headerName: "PR", width: 160 }, { field: "assignedto", headerName: "Assigned To", width: 180 }, { field: "status", headerName: "Status", width: 140 }, { field: "actions", headerName: "Create PO", width: 140, renderCell: (p) => <Button size="small" disabled={text(p.row.status).toLowerCase() === "po created"} onClick={() => createPoFromAssignment(p.row)}>Create PO</Button> }]} slots={{ toolbar: GridToolbar }} /></Paper><Paper sx={{ p: 2, mb: 2 }}><Stack direction={{ xs: "column", md: "row" }} spacing={2} sx={{ mb: 2 }}><TextField size="small" label="Terms and conditions" value={terms} onChange={(e) => setTerms(e.target.value)} /><TextField size="small" label="Warranty" value={warranty} onChange={(e) => setWarranty(e.target.value)} /><Button startIcon={<Refresh />} onClick={load}>Refresh</Button><Button variant="contained" disabled={!selectedPo || text(selectedPo.postatus).toLowerCase() !== "draft"} onClick={() => submitPo()}>Submit PO</Button><Button variant="contained" color="success" disabled={!selectedPo || text(selectedPo.postatus).toLowerCase() !== "submitted"} onClick={() => approvePo()}>Approve PO</Button><Button startIcon={<Print />} variant="outlined" disabled={!selectedPo} onClick={() => printPurchase2(selectedPo.poType === "Local" ? "localPo" : "po", { header: selectedPo, items: poItems })}>Print PO</Button></Stack><DataGrid autoHeight rows={pos.map((r) => ({ ...r, id: r._id }))} columns={[{ field: "poid", headerName: "PO", width: 160 }, { field: "prnumber", headerName: "PR", width: 160 }, { field: "vendor", headerName: "Vendor", flex: 1 }, { field: "postatus", headerName: "PO Status", width: 130 }, { field: "approvalStatus", headerName: "Approval", width: 130 }, { field: "currentStep", headerName: "Level", width: 90 }, { field: "actualAmount", headerName: "Amount", width: 120 }, { field: "actions", headerName: "Open", width: 100, renderCell: (p) => <Button size="small" onClick={() => openPo(p.row)}>Open</Button> }]} slots={{ toolbar: GridToolbar }} /></Paper>{selectedPo && <Paper sx={{ p: 2 }}><Typography variant="h6">PO items: {selectedPo.poid}</Typography><DataGrid autoHeight rows={poItems.map((r, i) => ({ ...r, id: i }))} columns={[{ field: "itemname", headerName: "Item", flex: 1 }, { field: "make", headerName: "Make", width: 130 }, { field: "quantity", headerName: "Qty", width: 100 }, { field: "price", headerName: "Rate", width: 110 }, { field: "gst", headerName: "GST", width: 90 }, { field: "total", headerName: "Total", width: 120 }]} /></Paper>}</Page>;
}
...
export function Purchase2QualityCheckPage() {
  ...
  const save = async () => {
    try {
      const qcno = key("QC");
      const header = await saveRow("storequalitycheckds2", { qcno, gatepassno: selected.gatepassno, poid: selected.poid, vendorname: selected.vendorname, storename: selected.storename, storeid: selected.storeid, checkedby: currentName(), checkedbyemail: currentUser(), checkdate: today(), status: "QC Done" });
      for (const item of items) {
