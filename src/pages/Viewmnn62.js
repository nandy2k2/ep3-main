...
import AddUserModal from './Addmnn62';
import AddUserModalBulk from './Addmnn62bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn62byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn62byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn62countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn62secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const hackathon=user.hackathon;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn62byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
