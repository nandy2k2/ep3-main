...
import AddUserModal from './Addmpublications';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepublicationsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'ugclisted',
headerName:'If listed in UGC care or scopus',
type:'dropdown',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
            ...
            return (
              ...
              >
                Check document
                  ...
                  <td>
                  <Button
                onClick={(e) => recognizeText(e, params.row)}
                variant="contained"
              >
                Check image
              </Button>
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpublicationsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpublicationscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpublicationssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const yop=user.yop;
const issn=user.issn;
const articlelink=user.articlelink;
const journallink=user.journallink;
const hindex=user.hindex;
const citation=user.citation;
const level=user.level;
const citationindex=user.citationindex;
const ugclisted=user.ugclisted;
const doclink=user.doclink;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatepublicationsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
doclink:doclink,
  ...
  const checktext1=(stext,itemstocheck)=> {
    ...
    const checktext=(stext)=> 
      {
        var kahan=0;
        var technologies=0;
        if(stext.toLowerCase().indexOf('kahan')>-1) {
          kahan=1;
        }
        if(stext.toLowerCase().indexOf('technol')>-1) {
          technologies=1;
        }
        if(kahan>0 && technologies>0) {
          alert('both found');
        } else if(kahan>0 || technologies>0) { 
          alert('One found');
