...
export default function StudentLedgerApprovalRolesPage() {
  ...
  const loadData = async () => {
    const res = await ep1.get(`/studentledgerapprovalroles?colid=${global1.colid}`);
  ...
  const saveData = async () => {
    ...
    if (editId) await ep1.post("/studentledgerapprovalroles-update", { ...payload, id: editId });
    else await ep1.post("/studentledgerapprovalroles", payload);
  ...
  const deleteData = async (id) => {
    if (!window.confirm("Delete this student ledger approval role?")) return;
    await ep1.post("/studentledgerapprovalroles-delete", { id });
  ...
  return (
    <Box p={3}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        <Typography variant="h5">Student Ledger Approval Roles</Typography>
