...
import AddUserModal from './Addmnn86';
import AddUserModalBulk from './Addmnn86bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn86byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn86byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn86countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn86secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const sanctioned=user.sanctioned;
const admitted=user.admitted;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn86byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
