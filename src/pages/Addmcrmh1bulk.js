...
return;
  ...
  const searchapi = async (rownumber,lead,company,designation,phone,email,address,city,state,country,pin,source,sourceemp,assignedto,assigneddate,followupdate,fcomments,leadstatus,year,product,amount,leadtype,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Lead' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Company' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Designation' + ',';
return;
}
if(!row[3]){
...
if(!row[9]){
f1=f1 + 'row ' + rownumber + '-' + 'Pin' + ',';
return;
}
if(!row[10]){
f1=f1 + 'row ' + rownumber + '-' + 'Source' + ',';
return;
}
if(!row[11]){
f1=f1 + 'row ' + rownumber + '-' + 'Added by' + ',';
return;
}
if(!row[12]){
...
f1=f1 + 'row ' + rownumber + '-' + 'Assigned date' + ',';
return;
}
 if(isNaN(new Date(row[14]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Follow up date' + ',';
return;
}
if(!row[15]){
f1=f1 + 'row ' + rownumber + '-' + 'Lead comments' + ',';
return;
}
if(!row[16]){
f1=f1 + 'row ' + rownumber + '-' + 'Lead status' + ',';
return;
}
if(!row[17]){
...
if(!row[18]){
f1=f1 + 'row ' + rownumber + '-' + 'Product' + ',';
return;
}
if(!row[19] || isNaN(row[19])){
f1=f1 + 'row ' + rownumber + '-' + 'Amount' + ',';
return;
}
if(!row[20]){
f1=f1 + 'row ' + rownumber + '-' + 'Lead type' + ',';
return;
}


            searchapi(rownumber,row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13],row[14],row[15],row[16],row[17],row[18],row[19],row[20],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      lead,company,designation,phone,email,address,city,state,country,pin,source,sourceemp,assignedto,assigneddate,followupdate,fcomments,leadstatus,year,product,amount,leadtype,status <br /><br />
