...
import AddUserModal from './Addmnn6clubs';
import AddUserModalBulk from './Addmnn6clubsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn6clubsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'club',
headerName:'Hackathon or Ideathon name',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn6clubsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn6clubscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn6clubssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const clubtype=user.clubtype;
const activity=user.activity;
const actdate=new Date(user.actdate);
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn6clubsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
