...
import AddUserModal from './Addmmtrialbalance2';
import AddUserModalBulk from './Addmmtrialbalance2bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtrialbalance2byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmtrialbalance2byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtrialbalance2countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtrialbalance2secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const credit=user.credit;
            ...
            const response =await ep1.get('/api/v2/updatemtrialbalance2byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
