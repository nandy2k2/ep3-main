...
return;
  ...
  const searchapi = async (rownumber,program,programcode,year,revision,revisionp,bosdate,doclink,type,level,status1) => {
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Is revised' + ',';
return;
}
if(!row[4] || isNaN(row[4])){
f1=f1 + 'row ' + rownumber + '-' + 'Revision percentage' + ',';
return;
}
 if(isNaN(new Date(row[5]))) {
f1=f1 + 'row ' + rownumber + '-' + 'BoS Date' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      program,programcode,year,revision,revisionp,bosdate,doclink,type,level,status <br /><br />
