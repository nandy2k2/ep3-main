...
return;
  ...
  const searchapi = async (rownumber,year,course,coursecode,title,submitdate,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createmdiscussionbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
return;
            ...
            searchapi(rownumber,row[0],row[1],row[2],row[3],row[4],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,course,coursecode,title,submitdate,status <br /><br />
