...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,participant,startup,actdate,sector,doclink,type,level,status1) => {
...
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Startup' + ',';
return;
}
 if(isNaN(new Date(row[5]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Foundation date' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,participant,startup,actdate,sector,doclink,type,level,status <br /><br />
