...
const VideoPage = () => {
  ...
  const [selectedVoice, setSelectedVoice] = useState(global1.selectedVoice);
  const [transitioning, setTransitioning] = useState(false);
  const playerRef = useRef(null);
  const progressTimerRef = useRef(null);

  const [videos, setVideos] = useState(global1.videos1);
  ...
  const fetchViewPage = async () => {
    ...
    setVideos(response.data.data.classes);
    console.log(videos);
    setCurrentIndex(0);
    //handleSpeakText();
  };
