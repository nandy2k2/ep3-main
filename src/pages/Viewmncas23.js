...
import AddUserModal from './Addmncas23';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v1/deletencas23byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v1/getncas23byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v1/getncas23countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v1/getncas23secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const score=user.score;
            ...
            const response =await ep1.get('/api/v1/updatencas23byfac', {
            params: {
            id: user._id,
            user: user,
            token:token,
