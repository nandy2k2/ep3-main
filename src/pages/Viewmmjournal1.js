...
import AddUserModal from './Addmmjournal1';
import AddUserModalBulk from './Addmmjournal1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemjournal1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmjournal1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmjournal1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmjournal1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const empid=user.empid;
            ...
            const response =await ep1.get('/api/v2/updatemjournal1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
