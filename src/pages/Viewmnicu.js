...
import AddUserModal from './Addmnicu';
import AddUserModalBulk from './Addmnicubulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenicubyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnicubyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnicucountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnicusecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const capacity=user.capacity;
            ...
            const response =await ep1.get('/api/v2/updatenicubyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
