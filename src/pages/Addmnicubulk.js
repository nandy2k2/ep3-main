...
return;
  ...
  const searchapi = async (rownumber,title,icucode,location,capacity,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnicubyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
return;
            ...
            searchapi(rownumber,row[0],row[1],row[2],row[3],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      title,icucode,location,capacity,status <br /><br />
