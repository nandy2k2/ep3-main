...
return;
  ...
  const searchapi = async (rownumber,itemgroup,item,type,category,amount,doccomments,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Item group' + ',';
return;
}
if(!row[1]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      itemgroup,item,type,category,amount,doccomments,status <br /><br />
