...
import AddUserModal from './Addmstorerequisationds';
import AddUserModalBulk from './Addmstorerequisationdsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletestorerequisationdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getstorerequisationdsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getstorerequisationdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getstorerequisationdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const poid=user.poid;
            ...
            const response =await ep1.get('/api/v2/updatestorerequisationdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
