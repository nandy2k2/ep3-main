...
return;
  ...
  const searchapi = async (rownumber,year,category,initialbudget,utilized,remaining,type,level,status1) => {
...
f1=f1 + 'row ' + rownumber + '-' + 'Initial budget' + ',';
return;
}
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'Utilized' + ',';
return;
}
if(!row[4] || isNaN(row[4])){
f1=f1 + 'row ' + rownumber + '-' + 'Remaining' + ',';
return;
}
if(!row[5]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,category,initialbudget,utilized,remaining,type,level,status <br /><br />
