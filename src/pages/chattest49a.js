...
const [filter1, setFilter1] = useState('"colid":' + global1.colid);
  ...
  const fetchBotResponse1 = async (userText) => {
    ...
    if(global1.chat=='Filter') {
      ...
      return {
        text: 'Add one or more options as per example - ' + ftext + '. Separate multiple filters by comma.',
      };
  ...
  const handleOperationClick = (button, currentCollection) => {
    ...
    if (button.id === 'bulk_upload') {
      ...
      setMessages(prev => [
        ...prev,
        { sender: 'user', text: `Clicked ${button.label}` },
        { sender: 'bot', text: 'Please type your query' }
      ]);
    }
  };
...
//   }
  ...
  const getformat = async (collectionToUse) => {
    ...
    if (!collection1) {
      setMessages(prev => [
        ...prev,
        { sender: 'bot', text: 'Please select a module first before getting format.' }
      ]);
      return;
    }
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        { sender: 'bot', text: 'Format ' + format + '. Format file downloaded successfully!' }
      ]);
    } catch (error) {
      setOpen(false);
      setMessages(prev => [
