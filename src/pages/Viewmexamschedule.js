...
import AddUserModal from './Addmexamschedule';
import AddUserModalBulk from './Addmexamschedulebulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteexamschedulebyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const schedule = async(e, row) => {
      e.stopPropagation();
      //do whatever you want with the row
      //alert(row._id);
      global1.exam=row.exam;
      global1.examcode=row.examcode;
      global1.examyear=row.year;
      global1.examsemester=row.semester;
      navigate('/dashmexamtimetable');
  };
...
return '';
          ...
          { field: 'actions', headerName: 'Actions', width: 230, renderCell: (params) => {
            return (
              ...
              <td>
              <Button
                onClick={(e) => schedule(e, params.row)}
                variant="contained"
              >
                Schedule
              </Button>
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getexamschedulebyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getexamschedulecountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getexamschedulesecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const semester=user.semester;
            ...
            const response =await ep1.get('/api/v2/updateexamschedulebyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
