...
export default function StudentLedgerAnalyticsPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError("");
      const res = await ep1.get("/api/v2/studentledgeranalytics", { params: buildParams() });
  ...
  const activeFilterText = useMemo(() => {
    ...
    if (!active.length) return "All ledger entries";
    return active.map((filter) => `${filterFields.find((item) => item.field === filter.field)?.label || filter.field}: ${filter.value}`).join(" | ");
  }, [filters]);
  ...
  return (
    <Box p={3}>
      ...
      <Stack className="no-print" direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2} sx={{ mb: 2 }}>
        <Box>
          <Typography variant="h5">Student Ledger Analytics</Typography>
          <Typography variant="body2" color="text.secondary">Dynamic totals and charts from student ledger</Typography>
        </Box>
      ...
      <Paper className="no-print" sx={{ p: 1, mb: 2, overflowX: "auto" }}>
        <DataGrid
          ...
          slotProps={{ toolbar: { showQuickFilter: true, csvOptions: { fileName: "student_ledger_analytics" } } }}
      ...
      <Box id="student-ledger-analytics-print" sx={{ bgcolor: "white", p: 3, border: "1px solid #ddd", maxWidth: "210mm", mx: "auto", color: "#111827", "@media print": { p: 0 } }}>
        <Stack alignItems="center" spacing={0.5} sx={{ mb: 1.5, textAlign: "center" }}>
          ...
          <Typography variant="subtitle1" fontWeight={800} sx={{ mt: 0.5 }}>Student Ledger Analytics</Typography>
