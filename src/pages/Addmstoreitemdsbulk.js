...
return;
  ...
  const searchapi = async (rownumber,storeid,storename,itemcode,itemname,quantity,type,status,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createstoreitemdsbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      storeid,storename,itemcode,itemname,quantity,type,status,status <br /><br />
