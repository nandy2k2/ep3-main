...
import AddUserModal from './Addmlpublications';
import AddUserModalBulk from './Addmlpublicationsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletelpublicationsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getlpublicationspublic', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getlpublicationscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getlpublicationssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const type=user.type;
            ...
            const response =await ep1.get('/api/v2/updatelpublicationsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
type:type,
  ...
  const onButtonClickgoe = async(e, row) => {
    ...
    //do whatever you want with the row
    //alert(row._id);
    global1.pubid=row._id;
    global1.publication=row.publication;
    // global1.faccoursename=row.coursename;
    // global1.faccoursecode=row.coursecode;
    navigate('/dashmlpubeditionspublic');
};
