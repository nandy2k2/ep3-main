...
const applicationToForm = (application = {}) => ({
  ...createInitialForm(),
  ...application,
  tenthsubjectmarks: application.tenthsubjectmarks?.length ? application.tenthsubjectmarks : [{ ...blankSubject }],
  twelvesubjectmarks: application.twelvesubjectmarks?.length ? application.twelvesubjectmarks : [{ ...blankSubject }],
  documents: application.documents || [],
  extraFields: application.extraFields || {}
});

export default function PublicAdmissionApplyTabbedProgramDraftPage() {
  const location = useLocation();
  const query = new URLSearchParams(location.search);
  const colid = query.get("colid");
  const formid = query.get("formid") || "default";
  const levelFromQuery = query.get("level") || "";
  const applicationIdFromQuery = query.get("applicationid") || "";
  ...
  const [applicationId, setApplicationId] = useState(applicationIdFromQuery);
  const [retrieveApplicationId, setRetrieveApplicationId] = useState(applicationIdFromQuery);
  ...
  useEffect(() => {
    if (colid && applicationIdFromQuery) {
      loadDraftApplication(applicationIdFromQuery);
    }
  }, [colid, applicationIdFromQuery]);
  ...
  const loadDraftApplication = async (nextApplicationId = retrieveApplicationId) => {
    const cleanApplicationId = String(nextApplicationId || "").trim();
    const cleanEmail = String(retrieveEmail || "").trim();
    const cleanPhone = String(retrievePhone || "").trim();
    if (!colid || (!cleanApplicationId && (!cleanEmail || !cleanPhone))) {
      setMessage("Please enter application number, or both email and phone to retrieve.");
      return;
    }
    try {
      setSaving(true);
      ...
      if (cleanApplicationId) {
        const res = await ep1.get("/admission-dynamic/application", {
          params: { colid, id: cleanApplicationId }
        });
        application = res.data || null;
      } else {
        const res = await ep1.get("/admission-dynamic/applications", {
          params: { colid, formid, email: cleanEmail, phone: cleanPhone }
        });
        const rows = Array.isArray(res.data) ? res.data : [];
        application = rows.find((item) => (
          String(item.email || "").trim().toLowerCase() === cleanEmail.toLowerCase() &&
      ...
      setRetrieveApplicationId(application._id || cleanApplicationId);
      setRetrieveEmail(application.email || cleanEmail);
      setRetrievePhone(application.phone || cleanPhone);
  ...
  const loadPrograms = async (year, type) => {
    ...
    setPrograms(sortProgramsByOrder(res.data || []));
  };
  ...
  const loadProgramsByType = async (year, types) => {
    ...
    setPrograms(sortedResponses.flatMap(([, items]) => items));
  };
  ...
  const applicationPayload = (status = "Draft") => ({
    ...
    englishmarks: 0,
    twelvesubjects: form.twelvesubjectmarks
  ...
  const saveDraft = async () => {
    ...
    if (!form.name || !form.email || !form.phone) {
      setMessage("Name, email and phone are required before saving a draft.");
      return null;
    }
    ...
    try {
      ...
      setRetrieveApplicationId(res.data?._id || "");
  ...
  const submitApplication = async () => {
    if (!form.academicyear || !form.programtype || !form.name || !form.email || !form.phone || !form.programcode) {
      setMessage("Please select year, type and program, and enter name, email and phone before submitting.");
      return;
    }
    if (!isDraftEditable) {
        ...
        <Grid container>
          {(rows.length ? rows : [{ subject: "-", marks: "-" }]).map((row, index) => (
            <React.Fragment key={`${title}-${index}`}>
              <Grid item xs={8} sx={{ borderTop: "1px solid #e5e7eb", px: 1.25, py: 0.75 }}>
                <Typography variant="body2">{row.subject || "-"}</Typography>
              </Grid>
              <Grid item xs={4} sx={{ borderTop: "1px solid #e5e7eb", px: 1.25, py: 0.75, textAlign: "right" }}>
  ...
  const documentsTabIndex = customTabs.length + 1;
  const selectedCustomTab = activeTab > 0 && activeTab < documentsTabIndex ? customTabs[activeTab - 1] : null;
  ...
  return (
    <Box sx={{ bgcolor: "#f5f7fb", minHeight: "100vh", py: 3 }}>
      <Grid container spacing={2} sx={{ maxWidth: 1100, mx: "auto", px: 2 }}>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="subtitle1" fontWeight={800} sx={{ mb: 1 }}>Retrieve Application and Continue Editing</Typography>
            <Stack direction={{ xs: "column", md: "row" }} spacing={1}>
              <TextField
                fullWidth
                label="Application number"
                value={retrieveApplicationId}
                onChange={(e) => setRetrieveApplicationId(e.target.value)}
                helperText={applicationId ? `Current application: ${applicationId}` : "Use application number, or enter email and phone below."}
              />
              <TextField
                fullWidth
                label="Email"
              ...
              />
              <Button variant="contained" disabled={saving} onClick={() => loadDraftApplication()} sx={{ minWidth: 150, height: 56 }}>
                Retrieve
              </Button>
            </Stack>
          </Paper>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            ...
            >
              ...
              {customTabs.map((tab) => (
                <Tab key={`${tab.page}-${tab.section}`} label={tab.label} />
              ))}
              <Tab label="Documents" />
            </Tabs>
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={1} justifyContent="space-between" sx={{ mt: 3 }}>
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
                <Button variant="outlined" onClick={saveDraft} disabled={saving || !isDraftEditable}>
                  {saving ? "Saving..." : "Save This Tab"}
                </Button>
                <Button
                  variant="outlined"
                ...
                <Button variant="contained" size="large" onClick={submitApplication} disabled={saving || !isDraftEditable}>
