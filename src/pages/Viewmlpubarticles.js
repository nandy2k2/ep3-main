...
import AddUserModal from './Addmlpubarticles';
import AddUserModalBulk from './Addmlpubarticlesbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletelpubarticlesbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
valueFormatter: params => dayjs(params?.value).format('DD/MM/YYYY'),
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getlpubarticlesbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getlpubarticlescountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getlpubarticlessecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
