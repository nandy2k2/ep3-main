...
import { Button, Box, Paper, Container, Grid, TextField, Typography } from '@mui/material';
import * as XLSX from 'xlsx';
import { saveAs } from 'file-saver';
import AddUserModal from './Addmmfeescol';
import AddUserModalBulk from './Addmmfeescolbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemfeescolbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      ...
      const response = await ep1.get('/api/v2/mfeescolbydate', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmfeescolcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmfeescolsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const feeitem=user.feeitem;
const semester=user.semester;
const feecategory=user.feecategory;
const paydate=user.paydate;
const amount=user.amount;
const paymode=user.paymode;
const payref=user.payref;
const paystatus=user.paystatus;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatemfeescolbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
      ...
      <React.Fragment>
        <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
            ...
            <Box display="flex" spacing={4}>
                        ...
                        From date
                        <TextField id="fromdate"  type="date" sx={{ width: "160px", marginRight:4, marginLeft: 4}}  variant="outlined" inputRef={fromdateref} /> 
                        To date 
                        <TextField id="todate"  type="date" sx={{ width: "160px", marginLeft: 4}}  variant="outlined" inputRef={todateref} />
                         <Button
             variant="contained"
             color="success"
             style={{ padding: '5px 10px', marginRight: '4px', marginLeft: '10px', fontSize: '12px', height: '55px', width: '80px' }}
