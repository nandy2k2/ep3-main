...
import AddUserModal from './Addmmindmaplist';
import AddUserModalBulk from './Addmmindmaplistbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemindmaplistbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClicknodes = async(e, row) => {
      e.stopPropagation();
      global1.mapid=row._id;
      
      navigate('/dashmmindmapnodes');
    };

    const onButtonClickedges = async(e, row) => {
      e.stopPropagation();
      global1.mapid=row._id;
      
      navigate('/dashmmindmapedges');
    };

    const onButtonClickmap = async(e, row) => {
      e.stopPropagation();
      global1.mapid=row._id;
      
      navigate('/viewmindmap');
    };
...
return '';
             ...
             <table>
              ...
              >
                Check document
                  ...
                  <td>
                  <Button
                onClick={(e) => onButtonClicknodes(e, params.row)}
                variant="contained"
              >
                Nodes
                  ...
                  <td>
                  <Button
                onClick={(e) => onButtonClickedges(e, params.row)}
                variant="contained"
              >
                Edges
                  ...
                  <td>
                  <Button
                onClick={(e) => onButtonClickmap(e, params.row)}
                variant="contained"
              >
                Mind map
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmindmaplistbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmindmaplistcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmindmaplistsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const type1=user.type1;
const level=user.level;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatemindmaplistbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
