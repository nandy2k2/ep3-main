...
export default function PublicAdmissionApplyPage() {
  ...
  useEffect(() => {
    if (colid && form.academicyear && form.programtype) {
      loadPrograms(form.academicyear, form.programtype);
    } else {
      setPrograms([]);
    }
  }, [colid, form.academicyear, form.programtype, admissionLevel]);
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={4}>
                <TextField select fullWidth required label="Type" value={form.programtype} onChange={(e) => updateProgramType(e.target.value)}>
