...
import AddUserModal from './Addmmassignments';
import AddUserModalBulk from './Addmmassignmentsbulk';
  ...
  const navigate = useNavigate();
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemassignmentsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
        //alert(lmsuser + ' ' + coursecode + ' ' + lmsyear + ' ' + colid);
      const response = await ep1.get('/api/v2/getmassignmentsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmassignmentscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmassignmentssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
    ...
    const submitassignment=async(assignmentid,assignment)=> {
      //alert(assignmentid + ' ' + assignment);
      global1.assignmentid=assignmentid;
      global1.assignment=assignment;
      navigate('/dashmmassignsubmit');
    }
...
const methodology=user.methodology;
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updatemassignmentsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
{rows.map((meetup3) => {
                return (
                  <div>
                    ...
                    Assignment: {meetup3.assignment}<br /><br />
                    Due date: {formatdates(meetup3.duedate)}<br /><br />
                    Description: <br /><br />
                    {meetup3.description}<br /><br />
                    Learning level: {meetup3.learning}<br /><br />
                    ...
                    {/* <TextToSpeech text={meetup3.voicetext} />  */}
                    {/* <br /><br /> */}
                    <Button
                onClick={(e) => submitassignment(meetup3._id,meetup3.assignment)}
                variant="contained"
