...
export default function IndIndentApprovalRolesPage() {
  ...
  const loadData = async () => {
    const res = await ep1.get(`/indindentapprovalroles?colid=${global1.colid}`);
  ...
  const saveData = async () => {
    ...
    if (editId) {
      await ep1.post("/indindentapprovalroles-update", { ...payload, id: editId });
    } else {
      await ep1.post("/indindentapprovalroles", payload);
    }
  ...
  const deleteData = async (id) => {
    ...
    await ep1.post("/indindentapprovalroles-delete", { id });
  ...
  return (
    <Box p={3}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        <Typography variant="h5">Indent Approval Roles</Typography>
        <Button
          variant="outlined"
