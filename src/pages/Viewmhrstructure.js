...
import AddUserModal from './Addmhrstructure';
import AddUserModalBulk from './Addmhrstructurebulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletehrstructurebyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
     ...
     const onButtonClicken = async(e, row) => {
        ...
        //do whatever you want with the row
        //alert(row._id);
        global1.structure=row.struture;
        global1.structureid=row._id;
        
        navigate('/dashmhrstructuresal');
    };
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gethrstructurebyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gethrstructurecountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gethrstructuresecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const handleOpenEdit1 =async (user) => {
            ...
            const struture=user.struture;
const description=user.description;
const businessrole=user.businessrole;
const paycommission=user.paycommission;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatehrstructurebyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
