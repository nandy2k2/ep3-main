...
import AddUserModal from './Addmmtestqnew';
import AddUserModalBulk from './Addmmtestqnewbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtestqnewbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const sectionid=global1.sectionid;
  
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmtestqnewbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtestqnewcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtestqnewsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatemtestqnewbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
    ...
    return (
      <React.Fragment>
        <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
          {/* Test id {testid} Sectionid {sectionid} <br /><br /> */}
