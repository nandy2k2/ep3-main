...
export default function CounterFee5PaymentPage() {
  ...
  const loadAccounts = async () => {
    try {
      const res = await ep1.get("/api/v2/dsgetaccountds", { params: { colid: global1.colid } });
      setAccounts((res.data?.data || []).sort((a, b) => String(a.account || "").localeCompare(String(b.account || ""), undefined, { numeric: true })));
    } catch (err) {
      setAccounts([]);
  ...
  const debitAccount = useMemo(() => accounts.find((account) => account._id === debitAccountId) || null, [accounts, debitAccountId]);
  const creditAccount = useMemo(() => accounts.find((account) => account._id === creditAccountId) || null, [accounts, creditAccountId]);

  const postJournalEntries = async (transactionid, totalAmount) => {
    if (!debitAccount || !creditAccount || !totalAmount) return;
    const base = {
      name: global1.name || selectedStudent?.name || "Counter Fee",
      user: global1.user,
      colid: global1.colid,
      year: selectedRows[0]?.academicyear || selectedStudent?.academicyear || "",
      transaction: "Counter Fee Receipt",
      transactionref: transactionid,
      subledger: selectedStudent?.name || selectedRows[0]?.student || "",
      cogs: "No",
      activitydate: paiddate,
      amount: totalAmount,
      student: selectedStudent?.name || selectedRows[0]?.student || "",
      regno: selectedStudent?.regno || selectedRows[0]?.regno || "",
      empid: "",
      status1: "Active",
      comments: transactionRemarks || `Counter fee receipt ${transactionid}`
    };
    await Promise.all([
      ep1.post("/api/v2/dscreatemjournal2", {
        ...base,
        accgroup: debitAccount.accountgroup || "",
        account: debitAccount.account || "",
        acctype: debitAccount.acctype || "",
        debit: totalAmount,
        credit: 0,
        type: "Debit"
      }),
      ep1.post("/api/v2/dscreatemjournal2", {
        ...base,
        accgroup: creditAccount.accountgroup || "",
        account: creditAccount.account || "",
        acctype: creditAccount.acctype || "",
        debit: 0,
        credit: totalAmount,
        type: "Credit"
      })
  ...
  const paySelected = async () => {
    ...
    if (!debitAccount || !creditAccount) {
      setError("Select debit account and credit account");
      return;
    }
    if (debitAccountId === creditAccountId) {
      setError("Debit account and credit account cannot be same");
      return;
    }
    try {
      setBusy(true);
      ...
      if (res.data.cheque) {
        ...
        await postJournalEntries(res.data.transactionid, amountReceived);
  ...
  return (
    <MenuPageShell title="Counter Fee 5">
      <Box p={3}>
        <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2} sx={{ mb: 2 }}>
          <Box>
            <Typography variant="h5" fontWeight={900}>Counter Fee 5</Typography>
            <Typography color="text.secondary">Search a student, collect selected past-due fees, and post debit/credit journal entries.</Typography>
          </Box>
          ...
          <Grid container spacing={1.5}>
            <Grid item xs={12} md={6}>
              <Autocomplete
                options={accounts}
                value={debitAccount}
                onChange={(_, value) => setDebitAccountId(value?._id || "")}
                getOptionLabel={(option) => option ? `${option.account || ""}${option.accountgroup ? ` | ${option.accountgroup}` : ""}` : ""}
                renderInput={(params) => <TextField {...params} size="small" label="Debit Account" />}
              />
            </Grid>
            <Grid item xs={12} md={3}>
              <TextField fullWidth size="small" label="Debit Account Group" value={debitAccount?.accountgroup || ""} InputProps={{ readOnly: true }} />
            </Grid>
            <Grid item xs={12} md={3}>
              <TextField fullWidth size="small" label="Debit Account Type" value={debitAccount?.acctype || ""} InputProps={{ readOnly: true }} />
            </Grid>
            <Grid item xs={12} md={6}>
              <Autocomplete
                options={accounts}
                value={creditAccount}
                onChange={(_, value) => setCreditAccountId(value?._id || "")}
                getOptionLabel={(option) => option ? `${option.account || ""}${option.accountgroup ? ` | ${option.accountgroup}` : ""}` : ""}
                renderInput={(params) => <TextField {...params} size="small" label="Credit Account" />}
              />
            </Grid>
            <Grid item xs={12} md={3}>
              <TextField fullWidth size="small" label="Credit Account Group" value={creditAccount?.accountgroup || ""} InputProps={{ readOnly: true }} />
            </Grid>
            <Grid item xs={12} md={3}>
              <TextField fullWidth size="small" label="Credit Account Type" value={creditAccount?.acctype || ""} InputProps={{ readOnly: true }} />
            </Grid>
