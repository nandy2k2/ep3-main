...
import AddUserModal from './Addmwebcourses';
import AddUserModalBulk from './Addmwebcoursesbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletewebcoursesbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getwebcoursesbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getwebcoursescountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getwebcoursessecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const applylink=user.applylink;
...
const type=user.type;
            ...
            const response =await ep1.get('/api/v2/updatewebcoursesbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
