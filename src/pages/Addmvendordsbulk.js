...
return;
  ...
  const searchapi = async (rownumber,vendorname,pan,gst,address,state,city,mobileno,email,type,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createvendordsbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      vendorname,pan,gst,address,state,city,mobileno,email,type,status <br /><br />
