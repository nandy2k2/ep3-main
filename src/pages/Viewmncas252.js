...
import AddUserModal from './Addmncas252';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v1/deletencas252byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'type',
headerName:'Type of Policy Document',
type:'dropdown',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v1/getncas252byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v1/getncas252countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v1/getncas252secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const score=user.score;
            ...
            const response =await ep1.get('/api/v1/updatencas252byfac', {
            params: {
            id: user._id,
            user: user,
            token:token,
