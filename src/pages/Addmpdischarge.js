...
if(transcript) {
dischargeref.current.value=transcript;
 }
 }
...
const discharge=dischargeref.current.value;
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createpdischargebyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
status1:'Submitted',
 ...
 <button onClick={setdischarge}>Set Discharge summary</button>
 <button onClick={setfollowup}>Set Follow up plan</button>
...
<TextField id="outlined-basic"  type="text" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={dischargeref} /><br /><br />

<LocalizationProvider dateAdapter={AdapterDayjs}><DatePicker label="Discharge date" inputRef={dischargedateref} sx={{ width: "100%"}} /></LocalizationProvider><br /><br />
