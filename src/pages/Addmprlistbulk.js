...
return;
  ...
  const searchapi = async (rownumber,template,templateid,category,faculty,facultyid,approveremail,level,finalstatus,type,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Template' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Template id' + ',';
return;
}
if(!row[2]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Approver email' + ',';
return;
}
...
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Final status' + ',';
return;
}
if(!row[8]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      template,templateid,category,faculty,facultyid,approveremail,level,finalstatus,type,status <br /><br />
