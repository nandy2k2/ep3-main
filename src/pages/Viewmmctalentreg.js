...
import AddUserModal from './Addmmctalentreg';
import AddUserModalBulk from './Addmmctalentregbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemctalentregbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmctalentregbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmctalentregcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmctalentregsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const gphone=user.gphone;
const payref=user.payref;
const payname=user.payname;
...
const type=user.type;
            ...
            const response =await ep1.get('/api/v2/updatemctalentregbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
