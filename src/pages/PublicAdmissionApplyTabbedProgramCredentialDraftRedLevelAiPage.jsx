...
const marksTypes = ["CGPA", "Percentage"];
const cbseGrades = ["A1", "A2", "B1", "B2", "C1", "C2", "D", "E1", "E2"];
const educationPanelMap = {
  educationpanel10th: { level: "10th", institutionField: "board_10th", marksTypeField: "marks_type_10th", marksField: "marks_10", cgpaField: "cgpa_10", title: "10th Details", institutionLabel: "Board" },
  educationpanel12th: { level: "12th", institutionField: "board_12th", marksTypeField: "marks_type_12th", marksField: "marks_12", cgpaField: "cgpa_12", title: "12th Details", institutionLabel: "Board" },
  educationpanelug: { level: "UG", institutionField: "University_UG", marksTypeField: "marks_type_UG", marksField: "marks_UG", cgpaField: "cgpa_UG", title: "UG Details", institutionLabel: "University" },
  educationpanelpg: { level: "PG", institutionField: "University_PG", marksTypeField: "marks_type_PG", marksField: "marks_PG", cgpaField: "cgpa_PG", title: "PG Details", institutionLabel: "University" }
};
const documentTypes = [
  "Photo",
  "Admit Card",
...
export default function PublicAdmissionApplyTabbedProgramCredentialDraftRedLevelAiPage() {
  ...
  const updateEducationPanel = (config, field, value) => {
    setForm((prev) => {
      ...
      if (field === config.marksTypeField) {
        if (value === "CGPA") next[config.marksField] = 0;
  ...
  const renderEducationPanel = (type, required = false) => {
    ...
    const showMarks = marksType === "Percentage";
    const showGrade = marksType === "CGPA";
    return (
      <Grid item xs={12}>
          ...
          <Grid container spacing={2}>
            ...
            {showMarks && (
              <Grid item xs={12} md={2.5}>
                <TextField fullWidth required={required} type="number" label="Marks / Percentage" value={form[config.marksField] || ""} onChange={(e) => updateEducationPanel(config, config.marksField, e.target.value)} />
              </Grid>
            )}
            {showGrade && (
              <Grid item xs={12} md={2.5}>
  ...
  const validateEducationPanels = () => {
    ...
    for (const field of activeEducationPanels) {
      ...
      const marksType = form[config.marksTypeField];
      const hasAnyValue = field.isrequired === "Yes" || form[config.institutionField] || marksType || form[config.marksField] || form[config.cgpaField];
