...
import AddUserModal from './Addmhdubed';
import AddUserModalBulk from './Addmhdubedbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletehdubedbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gethdubedbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gethdubedcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gethdubedsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const type=user.type;
            ...
            const response =await ep1.get('/api/v2/updatehdubedbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
