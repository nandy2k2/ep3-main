...
import AddUserModal from './Addmmpurchaseitems';
import AddUserModalBulk from './Addmmpurchaseitemsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletempurchaseitemsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmpurchaseitemsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmpurchaseitemscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmpurchaseitemssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const gst=user.gst;
const purchasetype=user.purchasetype;
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updatempurchaseitemsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
