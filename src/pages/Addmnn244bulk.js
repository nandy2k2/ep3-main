...
return;
  ...
  const searchapi = async (rownumber,faculty,designation,year,mooc,mplatform,certdate,mrole,type,level,status1) => {
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'MOOC course name' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'MOOC platform' + ',';
return;
}
 if(isNaN(new Date(row[5]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Certification date' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      faculty,designation,year,mooc,mplatform,certdate,mrole,type,level,status <br /><br />
