...
import AddUserModal from './Addmwcollection';
import AddUserModalBulk from './Addmwcollectionbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletewcollectionbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getwcollectionbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getwcollectioncountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getwcollectionsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const status=user.status;
            ...
            const response =await ep1.get('/api/v2/updatewcollectionbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
