...
import AddUserModal from './Addmtblemitter';
import AddUserModalBulk from './Addmtblemitterbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletetblemitterbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gettblemitterbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gettblemittercountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gettblemittersecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const domain=user.domain;
            ...
            const response =await ep1.get('/api/v2/updatetblemitterbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
