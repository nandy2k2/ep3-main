...
import AddUserModal from './Addmkeiyear';
import AddUserModalBulk from './Addmkeiyearbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletekeiyearbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
             ...
             <table>
              ...
              >
                Delete
                  ...
                  <td>
                  <Button
                onClick={(e) => onButtonClickgo1(e, params.row)}
                variant="contained"
              >
                Start appraisal
          ...
          const onButtonClickgo1 = async(e, row) => {
            e.stopPropagation();
            //do whatever you want with the row
            //alert(row._id);
            if(row.yearstatus=='Active') {

            } else {
                alert('This year appraisal is not active');
                return;
            }
            global1.appraisalyear=row.year;
            // global1.faccoursecode=row.coursecode;
            navigate('/keiteacherformsall1');
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getkeiyearbymy', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getkeiyearcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getkeiyearsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const yearstatus=user.yearstatus;
const role=user.role;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatekeiyearbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
