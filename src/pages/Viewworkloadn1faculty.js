...
import AddUserModal from './Addmworkloadn1';
import AddUserModalBulk from './Addmworkloadn1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteworkloadn1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
//  },
    ...
    const fetchViewPage = async (e) => {
      ...
      const response = await ep1.get('/api/v2/getworkloadn1faculty', {
        params: {
          token: token,
          colid: colid,
          facultyid: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getworkloadn1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getworkloadn1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updateworkloadn1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
