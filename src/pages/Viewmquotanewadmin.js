...
import AddUserModal from './Addmquotanew';
import AddUserModalBulk from './Addmquotanewbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletequotanewbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/quotanewdocs', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
        const response = await ep1.get('/api/v2/getquotanewcount', {
          params: {
            token: token,
            colid: colid,
            user: user
      ...
      const getgraphdatasecond = async () => {
        const response = await ep1.get('/api/v2/getquotanewsecond', {
          params: {
            token: token,
            colid: colid,
            user: user
...
const limit=user.limit;
            ...
            const response =await ep1.get('/api/v2/updatequotanewbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
<table width="600">
  <tr>
    ...
    <td width="20px"></td> 
    <td>
    <TextField id="outlined-basic"  type="password" sx={{ width: "100%"}} label="Keywords"  variant="outlined" inputRef={keywordsref} /><br /><br />
