...
export default function POApprovalRolesPage() {
  ...
  const loadData = async () => {
    const res = await ep1.get(`/poapprovalroles?colid=${global1.colid}`);
  ...
  const saveData = async () => {
    ...
    if (editId) {
      await ep1.post("/poapprovalroles-update", { ...payload, id: editId });
    } else {
      await ep1.post("/poapprovalroles", payload);
    }
  ...
  const deleteData = async (id) => {
    if (!window.confirm("Delete this PO approval role?")) return;
    await ep1.post("/poapprovalroles-delete", { id });
  ...
  return (
    <Box p={3}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        <Typography variant="h5">PO Approval Roles</Typography>
        <Button
          variant="outlined"
