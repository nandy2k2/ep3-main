...
import AddUserModal from './Addmstockregisterds';
import AddUserModalBulk from './Addmstockregisterdsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletestockregisterdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getstockregisterdsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getstockregisterdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getstockregisterdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const itemname=user.itemname;
const itemcode=user.itemcode;
const quantityadded=user.quantityadded;
const quantityreturn=user.quantityreturn;
const netquantity=user.netquantity;
const tdate=user.tdate;
const itemtype=user.itemtype;
const status=user.status;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatestockregisterdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
