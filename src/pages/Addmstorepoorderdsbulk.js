...
return;
  ...
  const searchapi = async (rownumber,year,vendor,vendorid,poid,price,description,returnamount,netprice,updatedate,postatus,status1) => {
...
f1=f1 + 'row ' + rownumber + '-' + 'Return amount' + ',';
return;
}
if(!row[7] || isNaN(row[7])){
f1=f1 + 'row ' + rownumber + '-' + 'Net price' + ',';
return;
}
 if(isNaN(new Date(row[8]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Update date' + ',';
return;
}
if(!row[9]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,vendor,vendorid,poid,price,description,returnamount,netprice,updatedate,postatus,status <br /><br />
