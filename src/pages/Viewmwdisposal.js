...
import AddUserModal from './Addmwdisposal';
import AddUserModalBulk from './Addmwdisposalbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletewdisposalbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getwdisposalbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getwdisposalcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getwdisposalsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const vendor=user.vendor;
            ...
            const response =await ep1.get('/api/v2/updatewdisposalbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
