...
export function GenerateTimetableAiPage() {
  ...
  const [server, setServer] = useState("");
  const [form, setForm] = useState({ academicyear: "", regulation: "", programcodes: [], semesters: [], startdate: "", enddate: "" });
  ...
  const confirm = async () => {
    try {
      ...
      setLoading(true);
      const res = await ep1.post(api.confirm, { colid: global1.colid, id: generated._id, user: global1.user });
