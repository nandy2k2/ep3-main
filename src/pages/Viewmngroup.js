...
import AddUserModal from './Addmngroup';
import AddUserModalBulk from './Addmngroupbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletengroupbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClickpages = async(e, row) => {
      e.stopPropagation();
      global1.group1=row.group1;

      navigate('/dashmngrouppages');
    };

    const onButtonClickaccr = async(e, row) => {
      e.stopPropagation();
      global1.group1=row.group1;

      navigate('/dashmngroupaccr');
    };
...
return '';
            ...
            return (
             <table>
                <tr>
                  <td>
                  <Button
                onClick={(e) => onButtonClickpages(e, params.row)}
                variant="contained"
              >
                Pages
              </Button>
                  ...
                  <td>
                  <Button
                onClick={(e) => onButtonClickaccr(e, params.row)}
                variant="contained"
              >
                Qualitative
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getngroupbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getngroupcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getngroupsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const handleOpenEdit1 =async (user) => {
            ...
            const group1=user.group1;
const fuser=user.fuser;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatengroupbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
