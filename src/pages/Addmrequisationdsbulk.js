...
return;
  ...
  const searchapi = async (rownumber,faculty,facultyid,itemcode,itemname,quantity,reqdate,allotted,allotdate,poid,storeid,storename,reqstatus,year,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Faculty' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Faculty id' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Item code' + ',';
return;
}
if(!row[3]){
...
f1=f1 + 'row ' + rownumber + '-' + 'Quantity' + ',';
return;
}
 if(isNaN(new Date(row[5]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Requisition date' + ',';
return;
}
if(!row[6] || isNaN(row[6])){
f1=f1 + 'row ' + rownumber + '-' + 'Allotted' + ',';
return;
}
 if(isNaN(new Date(row[7]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Allotment date' + ',';
return;
}
if(!row[8]){
f1=f1 + 'row ' + rownumber + '-' + 'POID' + ',';
return;
}
if(!row[9]){
f1=f1 + 'row ' + rownumber + '-' + 'Store id' + ',';
return;
}
if(!row[10]){
f1=f1 + 'row ' + rownumber + '-' + 'Store' + ',';
return;
}
if(!row[11]){
f1=f1 + 'row ' + rownumber + '-' + 'Requisition status' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      faculty,facultyid,itemcode,itemname,quantity,reqdate,allotted,allotdate,poid,storeid,storename,reqstatus,year,status <br /><br />
