...
return;
  ...
  const searchapi = async (rownumber,loation,type,spilldetails,spilldate,spilltime,actiontaken,status,status1) => {
...
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Spill details' + ',';
return;
}
 if(isNaN(new Date(row[3]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Spill date' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Spill time' + ',';
return;
}
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Ation taken' + ',';
return;
}
if(!row[6]){
...
return;
            ...
            searchapi(rownumber,row[0],row[1],row[2],row[3],row[4],row[5],row[6],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      loation,type,spilldetails,spilldate,spilltime,actiontaken,status,status <br /><br />
