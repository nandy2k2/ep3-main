...
import AddUserModal from './Addmwcolschedule';
import AddUserModalBulk from './Addmwcolschedulebulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletewcolschedulebyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getwcolschedulebyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getwcolschedulecountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getwcolschedulesecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const vendor=user.vendor;
            ...
            const response =await ep1.get('/api/v2/updatewcolschedulebyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
