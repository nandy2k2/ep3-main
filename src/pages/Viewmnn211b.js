...
import AddUserModal from './Addmnn211b';
import AddUserModalBulk from './Addmnn211bbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn211bbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const columns = [
     ...
     {
field:'committee',
headerName:'Selection Committee',
type:'text',
width:200,
editable:true,
...
{
field:'industryexp',
headerName:'Industry experience in years',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'acadexpyears',
headerName:'Academic experience in years',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn211bbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn211bcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn211bsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const comcode=user.comcode;
const year=user.year;
const university=user.university;
const faculty=user.faculty;
const isphd=user.isphd;
const rdate=new Date(user.rdate);
const designation=user.designation;
const salary=user.salary;
const ugc=user.ugc;
const industryexp=user.industryexp;
const acadexpyears=user.acadexpyears;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn211bbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
