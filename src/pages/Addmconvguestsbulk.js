...
return;
  ...
  const searchapi = async (rownumber,year,convid,convocation,guest,designation,organization,type,source,convday,attenddate,status1) => {
...
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Convocation' + ',';
return;
}
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Guest' + ',';
return;
}
if(!row[4]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Organization' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Guest type' + ',';
return;
}
if(!row[7]){
...
if(!row[8]){
f1=f1 + 'row ' + rownumber + '-' + 'Convocation day' + ',';
return;
}
 if(isNaN(new Date(row[9]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Attended date' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,convid,convocation,guest,designation,organization,type,source,convday,attenddate,status <br /><br />
