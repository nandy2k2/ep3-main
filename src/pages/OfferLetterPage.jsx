...
export default function OfferLetterPage() {
  ...
  const loadOptions = async () => {
    try {
      const res = await ep1.get("/api/v2/offer-letter/options", { params: { colid } });
  ...
  const searchStudents = async (source = filters) => {
    try {
      ...
      setError("");
      setLetterData(null);
      const res = await ep1.post("/api/v2/offer-letter/search", {
        colid,
  ...
  const generateLetter = async (regno = selectedRegno) => {
    ...
    try {
      ...
      setError("");
      const res = await ep1.get("/api/v2/offer-letter", { params: { colid, regno } });
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      <style>
        {`
          @media print {
            ...
            #offer-letter-print, #offer-letter-print * { visibility: visible; }
            #offer-letter-print { position: absolute; left: 0; top: 0; width: 190mm; min-height: 277mm; background: white; box-shadow: none !important; border: 0 !important; }
      ...
      <Stack className="no-print" direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2} sx={{ mb: 2 }}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Generate Offer Letter</Typography>
          <Typography variant="body2" color="text.secondary">Search students dynamically and print offer letters with fee structure from the fees master.</Typography>
        </Box>
      ...
      <Paper className="no-print" sx={{ p: 1, mb: 2, overflowX: "auto" }}>
        ...
        <DataGrid
          ...
          slotProps={{ toolbar: { showQuickFilter: true, csvOptions: { fileName: "offer_letter_students" } } }}
      ...
      {letterData && (
        <Paper className="no-print" sx={{ p: 1, mb: 2, overflowX: "auto" }}>
          <Typography variant="h6" sx={{ px: 1, py: 1 }}>Fee Structure from Fees Master</Typography>
          <DataGrid
            rows={fees}
            columns={feeColumns}
            autoHeight
            slots={{ toolbar: GridToolbar }}
            slotProps={{ toolbar: { showQuickFilter: true, csvOptions: { fileName: "offer_letter_fee_structure" } } }}
      ...
      <Box id="offer-letter-print" sx={{ bgcolor: "white", color: "#111827", maxWidth: "210mm", mx: "auto", p: 4, border: "1px solid #d1d5db", minHeight: "277mm", "@media print": { p: 0 } }}>
        {letterData ? (
          <>
            <Stack alignItems="center" spacing={0.5} sx={{ textAlign: "center", mb: 2 }}>
              ...
              <Typography variant="subtitle1" fontWeight={800} sx={{ mt: 1 }}>Offer Letter</Typography>
            </Stack>

            <Stack direction="row" justifyContent="space-between" sx={{ mb: 2 }}>
              <Typography variant="body2"><b>Ref:</b> OL/{student.regno}</Typography>
            ...
            <Typography variant="body2" sx={{ mb: 1.5, textAlign: "justify", lineHeight: 1.7 }}>
              We are pleased to offer you admission to the program shown below, subject to verification of original documents,
              fulfillment of eligibility conditions, payment of applicable fees, seat availability, and compliance with the rules and regulations of the institution.
            </Typography>
            ...
            <Grid container sx={{ border: "1px solid #cbd5e1", borderBottom: 0, fontSize: 12 }}>
              {["Fee Group", "Fee Item", "Semester", "Amount"].map((head, index) => (
                <Grid item xs={index === 1 ? 5 : index === 3 ? 2 : 2.5} key={head} sx={{ bgcolor: "#eef3f7", borderRight: index === 3 ? 0 : "1px solid #cbd5e1", borderBottom: "1px solid #cbd5e1", p: 0.75, fontWeight: 800, textAlign: index === 3 ? "right" : "left" }}>
                  {head}
                </Grid>
              ))}
              {fees.map((item) => (
                <React.Fragment key={item._id}>
                  <Grid item xs={2.5} sx={{ borderRight: "1px solid #cbd5e1", borderBottom: "1px solid #cbd5e1", p: 0.75 }}>{item.feegroup}</Grid>
                  <Grid item xs={5} sx={{ borderRight: "1px solid #cbd5e1", borderBottom: "1px solid #cbd5e1", p: 0.75 }}>{item.feeitem}</Grid>
                  <Grid item xs={2.5} sx={{ borderRight: "1px solid #cbd5e1", borderBottom: "1px solid #cbd5e1", p: 0.75 }}>{item.semester}</Grid>
                  <Grid item xs={2} sx={{ borderBottom: "1px solid #cbd5e1", p: 0.75, textAlign: "right" }}>{money(item.amount)}</Grid>
                </React.Fragment>
              ))}
              <Grid item xs={10} sx={{ bgcolor: "#eef3f7", borderRight: "1px solid #cbd5e1", borderBottom: "1px solid #cbd5e1", p: 0.75, fontWeight: 800 }}>Total</Grid>
              <Grid item xs={2} sx={{ bgcolor: "#eef3f7", borderBottom: "1px solid #cbd5e1", p: 0.75, textAlign: "right", fontWeight: 800 }}>{money(totals.amount)}</Grid>
            </Grid>

            <Typography variant="body2" sx={{ mt: 2, textAlign: "justify", lineHeight: 1.7 }}>
              This offer is valid subject to completion of the admission process within the prescribed timeline. Final admission will be confirmed only after
              document verification, fee payment, and satisfaction of all academic and statutory requirements.
            </Typography>
          ...
          <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 360, textAlign: "center" }}>
            ...
            <Typography variant="h6">Select a student and generate the offer letter</Typography>
          </Stack>
        )}
      </Box>
    </Container>
