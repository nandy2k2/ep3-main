...
import AddUserModal from './Addmconvattendees';
import AddUserModalBulk from './Addmconvattendeesbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteconvattendeesbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'ghrequired',
headerName:'Guest house required',
type:'dropdown',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getconvattendeesbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getconvattendeescountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getconvattendeessecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const convocation=user.convocation;
...
const feedate=new Date(user.feedate);
const refno=user.refno;
const feemode=user.feemode;
const ghrequired=user.ghrequired;
const apprstatus=user.apprstatus;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updateconvattendeesbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
