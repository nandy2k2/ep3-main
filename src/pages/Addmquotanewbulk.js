...
return;
  ...
  const searchapi = async (rownumber,type,subscription,limit,username,status1) => {
...
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Subscription' + ',';
return;
}
if(!row[2] || isNaN(row[2])){
f1=f1 + 'row ' + rownumber + '-' + 'Limit' + ',';
return;
}
if(!row[3]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      type,subscription,limit,username,status <br /><br />
