...
// }
    ...
    const fetchViewPage = async () => {
      //alert(classid);
      const regno1=regnoref.current.value;
      if(!regno1) {
        alert('Please enter reg no search');
        return;
      }
   
      //alert(ayear + ',' + pcode + ',' + semester + ',' + date1 + ',' + date2);
      const response = await ep1.get('/api/v2/getmiscorescom', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const handleExport = () => {
      ...
      for (var i=0; i<rows.length; i++) {
        Comment.push({
          ...
          student: rows[i]._id.regno,
          regno: rows[i]._id.test,
      ...
      saveAs(data, 'Internship_completed.xlsx');
      setOpenExport(false);
    };
