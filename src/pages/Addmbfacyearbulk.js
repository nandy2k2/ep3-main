...
return;
  ...
  const searchapi = async (rownumber,year,department,programcode,faculty,designation,qualification,pgyear,phdyear,doj,dol,experience,pan,unique,status1) => {
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Qualification' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'PG Year' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'PhD Year' + ',';
return;
}
 if(isNaN(new Date(row[8]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Date of joining' + ',';
return;
}
 if(isNaN(new Date(row[9]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Date of leaving' + ',';
return;
}
if(!row[10] || isNaN(row[10])){
f1=f1 + 'row ' + rownumber + '-' + 'Years of experience' + ',';
return;
}
if(!row[11]){
f1=f1 + 'row ' + rownumber + '-' + 'PAN' + ',';
return;
}
if(!row[12]){
f1=f1 + 'row ' + rownumber + '-' + 'Unique id' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,department,programcode,faculty,designation,qualification,pgyear,phdyear,doj,dol,experience,pan,unique,status <br /><br />
