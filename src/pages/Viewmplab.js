...
import AddUserModal from './Addmplab';
import AddUserModalBulk from './Addmplabbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteplabbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
valueFormatter: params => dayjs(params?.value).format('DD/MM/YYYY'),
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getplabbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getplabcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getplabsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const testdate=new Date(user.testdate);
            ...
            const response =await ep1.get('/api/v2/updateplabbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
