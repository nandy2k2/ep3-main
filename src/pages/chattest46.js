...
XLSX.writeFile(wb, 'format.xlsx');
  ...
  const bulkUploadData = async (data, operation) => {
    ...
    setOpen(true);
    try {
      const response = await ep1.post('/api/v2/bulkupload', {
      ...
      if (response.data.success) {
        setMessages(prev => [
          ...prev,
          { sender: 'bot', text: `Bulk ${operation} completed successfully! ${response.data.message}` }
        ]);
      } else {
        setMessages(prev => [
          ...prev,
          { sender: 'bot', text: `Bulk ${operation} failed: ${response.data.message}` }
        ]);
      }
    } catch (error) {
      setOpen(false);
      setMessages(prev => [
        ...prev,
        { sender: 'bot', text: `Error during bulk upload: ${error.message}` }
      ]);
    }
  };
