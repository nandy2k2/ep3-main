...
import AddUserModal from './Addmonlinepay';
import AddUserModalBulk from './Addmonlinepaybulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteonlinepaybyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
valueFormatter: params => dayjs(params?.value).format('DD/MM/YYYY'),
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getonlinepaybyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getonlinepaycountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getonlinepaysecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const bankref=user.bankref;
const paymode=user.paymode;
const bank=user.bank;
const amount=user.amount;
const username=user.username;
const regno=user.regno;
const clientcolid=user.clientcolid;
...
const paydate=new Date(user.paydate);
            ...
            const response =await ep1.get('/api/v2/updateonlinepaybyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
