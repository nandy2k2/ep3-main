...
import AddUserModal from './Addmstudawardsnew';
import AddUserModalBulk from './Addmstudawardsnewbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletestudawardsnewbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getstudawardsnewbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getstudawardsnewcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getstudawardsnewsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updatestudawardsnewbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
doclink:doclink,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.student + '~' + row.event;
