...
return '';
    ...
    const fetchViewPage = async () => {
      //alert(classid);
      const response = await ep1.get('/api/v2/getattendancenewbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    return (
      <React.Fragment>
        ...
        <Box display="flex" marginBottom={4} marginTop={2}>
          Class id {classid} date {formatdates(classdate)} year {year} course {course} {coursecode} program {program} {programcode} section {section} semester {semester}
          <br />
        </Box>
        <Box display="flex" marginBottom={4} marginTop={2}>
