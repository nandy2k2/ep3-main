...
import AddUserModal from './Addmexamroom';
import AddUserModalBulk from './Addmexamroombulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteexamroombyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getexamroombyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getexamroomcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getexamroomsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const seatno=user.seatno;
const roomno=user.roomno;
const roomseatno=user.roomseatno;
...
const photo=user.photo;
            ...
            const response =await ep1.get('/api/v2/updateexamroombyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
