...
import AddUserModal from './Addmmtestseenrol1';
import AddUserModalBulk from './Addmmtestseenrol1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtestseenrol1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const sessionid=global1.sessionid;
  
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmtestseenrol1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtestseenrol1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtestseenrol1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
