...
import AddUserModal from './Addmjobapplicationds';
import AddUserModalBulk from './Addmjobapplicationdsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletejobapplicationdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const jobid=global1.jobid;
    const jobtitle=global1.jobtitle;
  
    const fetchViewPage = async () => {
        const year=yearref.current.value;
        const status=statusref.current.value;
        if(!year || !status) {
            alert('Please select year and status');
            return;
        }
      const response = await ep1.get('/api/v2/jobapplicationbystatusyr', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getjobapplicationdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getjobapplicationdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const studentregno=user.studentregno;
const studentcv=user.studentcv;
const status=user.status;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatejobapplicationdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
status:status,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.studentname ;
        ...
        <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
           ...
           <Box display="flex" spacing={4}>
             ...
             <InputLabel id="year">Academic year</InputLabel>
                     ...
                     <InputLabel id="status" sx={{marginLeft: 3}}>Status</InputLabel>
             ...
             variant="contained"
             color="success"
             style={{ padding: '5px 10px', marginRight: '4px', marginLeft:6, fontSize: '12px', height: '40px', width: '80px' }}
