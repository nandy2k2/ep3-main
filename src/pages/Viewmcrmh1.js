...
import AddUserModal from './Addmcrmh1';
import AddUserModalBulk from './Addmcrmh1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletecrmh1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getcrmh1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getcrmh1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getcrmh1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const sourceemp=user.sourceemp;
...
const fcomments=user.fcomments;
...
const leadtype=user.leadtype;
            ...
            const response =await ep1.get('/api/v2/updatecrmh1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
