...
import AddUserModal from './Addmmassignsubmit';
import AddUserModalBulk from './Addmmassignsubmitbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemassignsubmitbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
//  },
 {
  ...
  width: 150,
  renderCell: (params) => 
    <a href={params.row.doclink} target="_blank">{params.row.doclink}</a>,
},
{
field:'ascomments',
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmassignsubmitbyass', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmassignsubmitcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmassignsubmitsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const submitdate=new Date(user.submitdate);
const doclink=user.doclink;
const ascomments=user.ascomments;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatemassignsubmitbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
{rows.map((meetup3) => {
                return (
                  <div>
                    ...
                    Student: {meetup3.student}<br /><br />
                    Reg no: {meetup3.regno}<br /><br />
                    Submit date: {formatdates(meetup3.submitdate)}<br /><br />
                    Description: <br /><br />
