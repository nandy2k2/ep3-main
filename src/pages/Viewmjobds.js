...
import AddUserModal from './Addmjobds';
import AddUserModalBulk from './Addmjobdsbulk';
...
function ViewPage() {
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClickapplicants = async(e, row) => {
            e.stopPropagation();
           
            global1.jobid=row._id;
            global1.jobtitle=row.title;
            
            navigate('/dashmjobapplicationds');
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletejobdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'lastapplieddate',
headerName:'Last application date',
type:'date',
width:200,
editable:false,
valueGetter: (params) => {
...
{
field:'joiningdate',
headerName:'Tentative joining date',
type:'text',
width:200,
editable:false,
valueFormatter: (params) => {
...
return '';
             ...
             <table>
              ...
              >
                Delete
                  ...
                  <td>
                  <Button
                onClick={(e) => onButtonClickapplicants(e, params.row)}
                variant="contained"
              >
                Applicants
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getjobdsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getjobdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getjobdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const lastapplieddate=new Date(user.lastapplieddate);
const joiningdate=user.joiningdate;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatejobdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
joiningdate:joiningdate,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.companyname;
