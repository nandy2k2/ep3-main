...
import AddUserModal from './Addmmassetassign';
import AddUserModalBulk from './Addmmassetassignbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemassetassignbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'username',
headerName:'Assigned to username',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmassetassignbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmassetassigncountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmassetassignsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const assetid=user.assetid;
const asset=user.asset;
const username=user.username;
const assigndate=new Date(user.assigndate);
const returndate=new Date(user.returndate);
const assetstatus=user.assetstatus;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatemassetassignbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
