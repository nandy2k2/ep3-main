...
import AddUserModal from './Addmlpubreviews';
import AddUserModalBulk from './Addmlpubreviewsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletelpubreviewsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getlpubreviewsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getlpubreviewscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getlpubreviewssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const handleOpenEdit1 =async (user) => {
            ...
            const articleid=user.articleid;
const peer=user.peer;
const designation=user.designation;
const rcomments=user.rcomments;
const submitdate=user.submitdate;
const peertye=user.peertye;
const commenttype=user.commenttype;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatelpubreviewsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
