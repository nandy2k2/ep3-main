...
import AddUserModal from './Addmhrsalstructure';
import AddUserModalBulk from './Addmhrsalstructurebulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletehrsalstructurebyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gethrsalstructurebyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gethrsalstructurecountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gethrsalstructuresecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const effectivedate=user.effectivedate;
const applieddate=user.applieddate;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatehrsalstructurebyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
