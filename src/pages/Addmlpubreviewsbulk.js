...
return;
  ...
  const searchapi = async (rownumber,articleid,peer,designation,rcomments,submitdate,peertye,commenttype,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Article id' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Peer name' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Peer designation' + ',';
return;
}
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Review comments' + ',';
return;
}
if(!row[4]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Peer type' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Comment type' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      articleid,peer,designation,rcomments,submitdate,peertye,commenttype,status <br /><br />
