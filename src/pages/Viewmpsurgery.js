...
import AddUserModal from './Addmpsurgery';
import AddUserModalBulk from './Addmpsurgerybulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepsurgerybyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpsurgerybyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpsurgerycountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpsurgerysecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const sdate=new Date(user.sdate);
const sstatus=user.sstatus;
const doccmments=user.doccmments;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatepsurgerybyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
