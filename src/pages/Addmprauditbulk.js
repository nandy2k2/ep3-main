...
return;
  ...
  const searchapi = async (rownumber,pr,prid,faculty,facultyid,activitydate,activity,type,level,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'PR' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'PR ID' + ',';
return;
}
if(!row[2]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      pr,prid,faculty,facultyid,activitydate,activity,type,level,status <br /><br />
