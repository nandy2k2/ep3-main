...
return;
  ...
  const searchapi = async (rownumber,year,convid,convocation,guesthouse,room,roomtype,guest,clogin,checkin,checkout,type,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Aademic year' + ',';
return;
}
if(!row[1]){
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Guest house' + ',';
return;
}
if(!row[4]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Room type' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Guest name' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Guest login' + ',';
return;
}
 if(isNaN(new Date(row[8]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Checkin date' + ',';
return;
}
 if(isNaN(new Date(row[9]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Checkout date' + ',';
return;
}
if(!row[10]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,convid,convocation,guesthouse,room,roomtype,guest,clogin,checkin,checkout,type,status <br /><br />
