...
import AddUserModal from './Addmmcourseslist';
import AddUserModalBulk from './Addmmcourseslistbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemcourseslistbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmcourseslistbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmcourseslistcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmcourseslistsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const credit=user.credit;
            ...
            const response =await ep1.get('/api/v2/updatemcourseslistbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
