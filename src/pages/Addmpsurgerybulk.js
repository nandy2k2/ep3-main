...
return;
  ...
  const searchapi = async (rownumber,patient,puser,admid,surgery,doctor,doctorid,sdate,sstatus,doccmments,status1) => {
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Doctor ID' + ',';
return;
}
 if(isNaN(new Date(row[6]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Surgery date' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Surgery status' + ',';
return;
}
if(!row[8]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      patient,puser,admid,surgery,doctor,doctorid,sdate,sstatus,doccmments,status <br /><br />
