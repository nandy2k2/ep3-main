...
import AddUserModal from './Addmwspill1';
import AddUserModalBulk from './Addmwspill1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletewspill1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getwspill1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getwspill1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getwspill1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const handleOpenEdit1 =async (user) => {
            ...
            const loation=user.loation;
const type=user.type;
const binid=user.binid;
const spilldetails=user.spilldetails;
const spilldate=new Date(user.spilldate);
const spilltime=user.spilltime;
const actiontaken=user.actiontaken;
const status=user.status;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatewspill1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
