...
import AddUserModal from './Addmbfacyear';
import AddUserModalBulk from './Addmbfacyearbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletebfacyearbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getbfacyearbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getbfacyearcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getbfacyearsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const pgyear=user.pgyear;
const phdyear=user.phdyear;
const doj=new Date(user.doj);
const dol=new Date(user.dol);
const experience=user.experience;
const pan=user.pan;
const unique=user.unique;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatebfacyearbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
