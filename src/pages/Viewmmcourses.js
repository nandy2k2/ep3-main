...
import AddUserModal from './Addmmcourses';
import AddUserModalBulk from './Addmmcoursesbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemcoursesbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmcoursesbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmcoursescountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmcoursessecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const credit=user.credit;
            ...
            const response =await ep1.get('/api/v2/updatemcoursesbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
