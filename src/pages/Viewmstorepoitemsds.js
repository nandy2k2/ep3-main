...
import AddUserModal from './Addmstorepoitemsds';
import AddUserModalBulk from './Addmstorepoitemsdsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletestorepoitemsdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getstorepoitemsdsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getstorepoitemsdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getstorepoitemsdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const itemname=user.itemname;
            ...
            const response =await ep1.get('/api/v2/updatestorepoitemsdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
