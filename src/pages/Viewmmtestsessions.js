...
import AddUserModal from './Addmmtestsessions';
import AddUserModalBulk from './Addmmtestsessionsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtestsessionsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClickenrol = async(e, row) => {
      e.stopPropagation();
      global1.sessionid=row._id;
      global1.session =row.session;
      
      
      navigate('/dashmmtestseenrol1');
    };
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmtestsessionsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtestsessionscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtestsessionssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatemtestsessionsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
