...
import AddUserModal from './AddmPatient';
import AddUserModalBulk from './AddmPatientbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletePatientbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
valueFormatter: params => dayjs(params?.value).format('DD/MM/YYYY'),
    ...
    //do whatever you want with the row
    //alert(row._id);
    global1.patient=row.patient;
    global1.puser=row.username;
    alert('Patient ' + global1.patient + ' selected');
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getPatientbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getPatientcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getPatientsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const dob=new Date(user.dob);
            ...
            const response =await ep1.get('/api/v2/updatePatientbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
