...
import { Button, Box, Paper, Container, TextField, Grid } from '@mui/material';
...
function ViewPage() {
  const navigate = useNavigate();
    ...
    const onButtonClickas = async(e, row) => {
      e.stopPropagation();
      global1.assignment=row.assignment;
      global1.assignmentid=row._id;
      navigate('/dashmmassignsubmitfac');
  ...
  const onButtonClickm = async() => {
    ...
    const response = await epai1.get('/api/v1/getresponse2', {
        params: {
            user:user,
            colid:colid,
            prompt:'Create assignment for ' + coursename + ' with focus on ' + keywords
        }
  ...
  element.download = 'assignment_' + coursename + ".html";
