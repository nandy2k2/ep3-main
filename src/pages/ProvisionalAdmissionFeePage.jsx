...
const ProvisionalAdmissionFeePage = () => {
  ...
  const loadOptions = async () => {
    const res = await ep1.get("/api/v2/provisionaladmissionfee/options", { params: { colid } });
  ...
  const loadRows = async () => {
    ...
    setError("");
    try {
      const res = await ep1.get("/api/v2/provisionaladmissionfee", { params: { colid } });
      setRows(res.data.data || []);
    } catch (err) {
      setError(err.response?.data?.message || err.message || "Unable to load provisional admission fees");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveRow = async () => {
    ...
    try {
      ...
      const res = form.id
        ? await ep1.post("/api/v2/provisionaladmissionfee/update", payload)
        : await ep1.post("/api/v2/provisionaladmissionfee", payload);
      setMessage(form.id ? "Provisional admission fee updated." : "Provisional admission fee saved.");
      setForm(emptyForm);
      await loadRows();
      await loadOptions();
      return res.data.data;
    } catch (err) {
      setError(err.response?.data?.message || err.message || "Unable to save provisional admission fee");
      return null;
    } finally {
      setSaving(false);
    }
  ...
  const deleteRow = async (row) => {
    if (!window.confirm("Delete this provisional admission fee?")) return;
    setSaving(true);
    setError("");
    setMessage("");
    try {
      await ep1.post("/api/v2/provisionaladmissionfee/delete", { id: row._id, colid });
      setMessage("Provisional admission fee deleted.");
      await loadRows();
    } catch (err) {
      setError(err.response?.data?.message || err.message || "Unable to delete provisional admission fee");
    } finally {
      setSaving(false);
    }
  };
  ...
  const downloadTemplate = () => {
    const sample = [{
      ...
      amount: 1000,
      description: "Provisional admission fee details"
    }];
    const ws = XLSX.utils.json_to_sheet(sample);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Provisional Admission Fee");
    XLSX.writeFile(wb, "Provisional_Admission_Fee_Template.xlsx");
  };
  ...
  const readExcel = (event) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        const res = await ep1.post("/api/v2/provisionaladmissionfee/bulk", { colid, user: currentUser, rows: jsonRows });
  ...
  const columns = [
    ...
    { field: "description", headerName: "Description", minWidth: 320, flex: 1 },
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", sm: "center" }} spacing={2} sx={{ mb: 2 }}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Provisional Admission fee</Typography>
          <Typography variant="body2" color="text.secondary">Configure provisional admission fee descriptions by academic year and program.</Typography>
        </Box>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: "wrap" }}>
          <Button variant="contained" startIcon={<Save />} disabled={saving || !form.academicyear || !form.program || !form.programcode} onClick={saveRow}>
            {form.id ? "Update" : "Save"}
          </Button>
...
export default ProvisionalAdmissionFeePage;
