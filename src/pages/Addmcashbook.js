...
const levelref=useRef();
    ...
    const {
      ...
      const setcashnook=(id)=> {
if(transcript) {
cashnookref.current.value=transcript;
 }
 }
...
const cashnook=cashnookref.current.value;
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createcashbookbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
status1:'Submitted',
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      <p>{transcript}</p>
       ...
       <button onClick={setcashnook}>Set Cashbook</button>
 ...
 <button onClick={setlevel}>Set Level</button>
    ...
    <p>Cashbook</p>
<TextField id="outlined-basic"  type="text" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={cashnookref} /><br /><br />
