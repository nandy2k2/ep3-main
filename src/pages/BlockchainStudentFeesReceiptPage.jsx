...
export default function BlockchainStudentFeesReceiptPage() {
  ...
  const receiptKey = useMemo(() => {
    if (!selectedStudent.regno || !selectedRows.length) return "";
    return selectedRows.map((row) => row._id).sort().join("-");
  }, [selectedRows, selectedStudent.regno]);

  const receiptNumber = useMemo(() => {
    if (!selectedStudent.regno || !receiptKey) return "";
    return `BFR-${selectedStudent.regno}-${receiptKey.replace(/[^a-zA-Z0-9]/g, "").slice(-10).toUpperCase()}`;
  }, [selectedStudent.regno, receiptKey]);

  const verificationUrl = useMemo(() => {
    if (!selectedStudent.regno) return `${window.location.origin}/verify-blockchain-fees-receipt`;
    const params = new URLSearchParams({ regno: selectedStudent.regno });
    return `${window.location.origin}/verify-blockchain-fees-receipt?${params.toString()}`;
  ...
  const storeReceiptBlockchain = async () => {
    if (!selectedRows.length || !selectedStudent.regno) {
      setError("Please select student and fee items before storing receipt in blockchain");
      return;
    }
    if (blockchainSaving) return;
    try {
      setBlockchainSaving(true);
      setError("");
      setBlockchainMessage("");
      const res = await ep1.post("/api/v2/studentfeesreceipt/blockchain-store", {
        colid: global1.colid,
        user: global1.user,
      ...
      setBlockchainMessage(`${res.data?.message || "Fees receipt stored in blockchain"}. Hash: ${res.data?.data?.hash || ""}`);
    } catch (err) {
      setError(err.response?.data?.message || "Unable to store fees receipt in blockchain");
    } finally {
  ...
  return (
    <Box p={3}>
      ...
      <Stack className="no-print" direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2} sx={{ mb: 2 }}>
        <Box>
          <Typography variant="h5">Blockchain Fees Receipt</Typography>
          <Typography variant="body2" color="text.secondary">Select student and fee items to generate printable receipt and store it in blockchain</Typography>
        </Box>
        <Stack direction="row" spacing={1}>
          <Button
            ...
            onClick={storeReceiptBlockchain}
            disabled={blockchainSaving || !selectedRows.length}
          >
      ...
      {blockchainMessage && <Alert className="no-print" severity="success" sx={{ mb: 2 }} onClose={() => setBlockchainMessage("")}>{blockchainMessage}</Alert>}

      <Paper className="no-print" sx={{ p: 2, mb: 2 }}>
        <Typography variant="body2" sx={{ mb: 2, overflowWrap: "anywhere" }}>
          <b>Verification URL:</b> {verificationUrl}
        </Typography>
      ...
      <Box id="fees-receipt-print" sx={{ bgcolor: "white", p: 3, border: "1px solid #ddd", maxWidth: "210mm", mx: "auto", color: "#111827", "@media print": { p: 0 } }}>
        <Stack alignItems="center" spacing={0.5} sx={{ mb: 1.5, textAlign: "center" }}>
          ...
          {receiptNumber && <Typography variant="body2"><b>Receipt No:</b> {receiptNumber}</Typography>}
        </Stack>
        ...
        <Box sx={{ mt: 2, p: 1, border: "1px solid #cbd5e1", bgcolor: "#f8fafc" }}>
          <Typography variant="body2" fontWeight={800}>Blockchain Verification</Typography>
          <Typography variant="body2" sx={{ overflowWrap: "anywhere" }}>{verificationUrl}</Typography>
        </Box>
