...
import AddUserModal from './Addmconvdocs';
import AddUserModalBulk from './Addmconvdocsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteconvdocsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getconvdocsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getconvdocscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getconvdocssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const type=user.type;
            ...
            const response =await ep1.get('/api/v2/updateconvdocsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
