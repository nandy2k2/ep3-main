...
import AddUserModal from './Addmpadmission';
import AddUserModalBulk from './Addmpadmissionbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepadmissionbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    //do whatever you want with the row
    //alert(row._id);
    const d1=new Date(row.admdate);
  const year=d1.getFullYear();
  var month=d1.getMonth() + 1;
  var date1=d1.getDate();
  // const datestr=year + '-' + month + '-' + date1;
  const datestr=date1 + '-' + month + '-' + year;
  
    global1.padmid=datestr;
    global1.admid=datestr;
    alert('Patient admission ' + global1.padmid + ' selected');
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpadmissionbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpadmissioncountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpadmissionsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const admdate=new Date(user.admdate);
const admtime=user.admtime;
const admstatus=user.admstatus;
const doctor=user.doctor;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatepadmissionbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
    ...
    return (
      <React.Fragment>
        <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
          Selected Patient {patient1} user id {puser} <br /> <br />
