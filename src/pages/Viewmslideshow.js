...
import AddUserModal from './Addmslideshow';
import AddUserModalBulk from './Addmslideshowbulk';
...
function ViewPage() {
  ...
  const handleImageUpload = (event) => {
    ...
    const voicetext=global1.voicetext;
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteslideshowbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
      ...
      const getrowsec=()=> {
        alert(rows[0].imagelink + ' ' + rows[0].duration);
      }

      const assign=()=> {
        var c="image src='" + rows[rowcount].imagelink + "' />";
      ...
      const starttimer=() => {
        ...
        const timerId = setInterval(() => {
          ...
          if(count==rows[rowcount].duration) {
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getslideshowbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getslideshowcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getslideshowsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
