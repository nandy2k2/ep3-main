...
return;
  ...
  const searchapi = async (rownumber,year,accgroup,account,acctype,transaction,transactionref,subledger,cogs,activitydate,amount,type,student,regno,empid,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createmjournal1byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
return;
            ...
            searchapi(rownumber,row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,accgroup,account,acctype,transaction,transactionref,subledger,cogs,activitydate,amount,type,student,regno,empid,status <br /><br />
