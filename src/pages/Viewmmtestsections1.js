...
import AddUserModal from './Addmmtestsections1';
import AddUserModalBulk from './Addmmtestsections1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtestsections1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClickq = async(e, row) => {
      e.stopPropagation();
      global1.sectionid=row._id;
      global1.section =row.section;
      
      
      navigate('/dashmmtestqnew');
    };
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmtestsections1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtestsections1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtestsections1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatemtestsections1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
