...
return;
  ...
  const searchapi = async (rownumber,year,scholarship,type,student,regno,amount,doclink,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createscholnewstudbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Scholarship' + ',';
return;
}
if(!row[2]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,scholarship,type,student,regno,amount,doclink,status <br /><br />
