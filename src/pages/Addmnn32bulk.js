...
return;
  ...
  const searchapi = async (rownumber,year,library,eresource,erno,expenditure,doclink,type,level,status1) => {
...
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'E resources' + ',';
return;
}
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'E resource no' + ',';
return;
}
if(!row[4] || isNaN(row[4])){
f1=f1 + 'row ' + rownumber + '-' + 'Annual expenditure' + ',';
return;
}
if(!row[5]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,library,eresource,erno,expenditure,doclink,type,level,status <br /><br />
