...
const [link, setLink] = useState();
    ...
    const testid1=global1.testid;
    const sectionid1=global1.sectionid;
...
return;
  ...
  const searchapi = async (rownumber,testid,sectionid,question,optiona,optionb,optionc,optiond,correcta,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createmtestqnewbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Testid' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Section id' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Question' + ',';
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Option A' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Option B' + ',';
return;
}
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Option C' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Option D' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Correct Option' + ',';
return;
}
if(!row[8]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      testid,sectionid,question,optiona,optionb,optionc,optiond,correcta,type,level,status <br /><br />
