...
return;
  ...
  const searchapi = async (rownumber,year,convid,convocation,guest,clogin,from,to,traveldate,type,status1) => {
...
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Convoation id' + ',';
return;
}
if(!row[2]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'From location' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'To location' + ',';
return;
}
 if(isNaN(new Date(row[7]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Travel date' + ',';
return;
}
if(!row[8]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,convid,convocation,guest,clogin,from,to,traveldate,type,status <br /><br />
