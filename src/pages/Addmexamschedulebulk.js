...
return;
  ...
  const searchapi = async (rownumber,year,exam,examcode,fromdate,todate,semester) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createexamschedulebyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
return;
    ...
    //handleClose();
    alert('Thank you. Valid rows will be updated in thirty minutes.');
    //handleClose();
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      Please upload excel file with data in proper format with following columns in order.<br /><br />
                        exam,question,option,score,student,regno,scolid,status <br /><br />
