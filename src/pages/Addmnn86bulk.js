...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,sanctioned,admitted,applied,doclink,type,level,status1) => {
...
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'Sanctioned seat' + ',';
return;
}
if(!row[4] || isNaN(row[4])){
f1=f1 + 'row ' + rownumber + '-' + 'Admitted' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'Applied' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,sanctioned,admitted,applied,doclink,type,level,status <br /><br />
