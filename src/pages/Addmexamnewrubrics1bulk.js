...
return;
  ...
  const searchapi = async (rownumber,examcode,student,regno,program,programcode,course,coursecode,semester,credits,midtermscore,assignmentmarks,presentationmarks,testmarks,attendancemarks,type,level,status1) => {
...
f1=f1 + 'row ' + rownumber + '-' + 'Credits' + ',';
return;
}
if(!row[9] || isNaN(row[9])){
f1=f1 + 'row ' + rownumber + '-' + 'Mid term score' + ',';
return;
}
if(!row[10] || isNaN(row[10])){
f1=f1 + 'row ' + rownumber + '-' + 'Assignment marks' + ',';
return;
}
if(!row[11] || isNaN(row[11])){
f1=f1 + 'row ' + rownumber + '-' + 'Presentation marks' + ',';
return;
}
if(!row[12] || isNaN(row[12])){
f1=f1 + 'row ' + rownumber + '-' + 'Test marks' + ',';
return;
}
if(!row[13] || isNaN(row[13])){
f1=f1 + 'row ' + rownumber + '-' + 'Attendancemarks' + ',';
return;
}
if(!row[14]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      examcode,student,regno,program,programcode,course,coursecode,semester,credits,midtermscore,assignmentmarks,presentationmarks,testmarks,attendancemarks,type,level,status <br /><br />
