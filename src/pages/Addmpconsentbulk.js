...
return;
  ...
  const searchapi = async (rownumber,patient,puser,admid,title,description,consentby,relation,cdate,link,doccomments,status1) => {
...
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Patient user' + ',';
return;
}
if(!row[2]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Consent by' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Relation' + ',';
return;
}
 if(isNaN(new Date(row[7]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Consent date' + ',';
return;
}
if(!row[8]){
f1=f1 + 'row ' + rownumber + '-' + 'Link' + ',';
return;
}
if(!row[9]){
f1=f1 + 'row ' + rownumber + '-' + 'Doctor comments' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      patient,puser,admid,title,description,consentby,relation,cdate,link,doccomments,status <br /><br />
