...
import AddUserModal from './Addmrequisationds';
import AddUserModalBulk from './Addmrequisationdsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleterequisationdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getrequisationdsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getrequisationdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getrequisationdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const reqdate=new Date(user.reqdate);
const allotted=user.allotted;
const allotdate=new Date(user.allotdate);
const poid=user.poid;
const storeid=user.storeid;
const storename=user.storename;
const reqstatus=user.reqstatus;
const year=user.year;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updaterequisationdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
