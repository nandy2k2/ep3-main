...
function ViewPage() {
  ...
  const handleImageUpload = (event) => {
    ...
    const onButtonClicken = async(e, row) => {
        ...
        //do whatever you want with the row
        //alert(row._id);
        global1.payyear=row.year;
        global1.receiptnumber=row._id;
        global1.paydate=row.paydate;
        global1.studentname=row.student;
        global1.regno=row.regno;
        global1.paymode=row.paymode;
        global1.amount=row.amount;
        global1.feegroup=row.feegroup;
        global1.feeitem=row.feeitem;
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmfeescolbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
