...
return;
  ...
  const searchapi = async (rownumber,year,convid,convocation,student,regno,program,passingyear,login,password,rank,grade,cgpa,award,fees,feedate,refno,feemode,ghrequired,apprstatus,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Academi year' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'convocation id' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Convoation' + ',';
return;
}
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Student' + ',';
...
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Passing year' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Login' + ',';
return;
}
if(!row[8]){
...
if(!row[9]){
f1=f1 + 'row ' + rownumber + '-' + 'Rank' + ',';
return;
}
if(!row[10]){
f1=f1 + 'row ' + rownumber + '-' + 'Grade' + ',';
return;
}
if(!row[11] || isNaN(row[11])){
f1=f1 + 'row ' + rownumber + '-' + 'CGPA' + ',';
return;
}
if(!row[12]){
f1=f1 + 'row ' + rownumber + '-' + 'Award' + ',';
return;
}
if(!row[13] || isNaN(row[13])){
f1=f1 + 'row ' + rownumber + '-' + 'Fees' + ',';
return;
}
 if(isNaN(new Date(row[14]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Fees paid date' + ',';
return;
}
if(!row[15]){
f1=f1 + 'row ' + rownumber + '-' + 'Payment Ref no' + ',';
return;
}
if(!row[16]){
f1=f1 + 'row ' + rownumber + '-' + 'Fee mode' + ',';
return;
}
if(!row[17]){
f1=f1 + 'row ' + rownumber + '-' + 'Guest house required' + ',';
return;
}
if(!row[18]){
f1=f1 + 'row ' + rownumber + '-' + 'Approval status' + ',';
return;
}


            searchapi(rownumber,row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13],row[14],row[15],row[16],row[17],row[18],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,convid,convocation,student,regno,program,passingyear,login,password,rank,grade,cgpa,award,fees,feedate,refno,feemode,ghrequired,apprstatus,status <br /><br />
