...
import AddUserModal from './Addmstorepoorderds';
import AddUserModalBulk from './Addmstorepoorderdsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletestorepoorderdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getstorepoorderdsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getstorepoorderdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getstorepoorderdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const returnamount=user.returnamount;
const netprice=user.netprice;
const updatedate=new Date(user.updatedate);
const postatus=user.postatus;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatestorepoorderdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
