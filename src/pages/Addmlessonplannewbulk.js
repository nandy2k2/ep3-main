...
return;
  ...
  const searchapi = async (rownumber,academicyear,program,programcode,course,coursecode,semester,section,module,lessonno,lesson,lessonplan,noofperiod,fromdate,todate,actualdate,topics,status1) => {
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Course ID' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Course code' + ',';
return;
}
if(!row[5]){
...
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Section' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Module' + ',';
return;
}
if(!row[8] || isNaN(row[8])){
f1=f1 + 'row ' + rownumber + '-' + 'Lesson no' + ',';
return;
}
if(!row[9]){
f1=f1 + 'row ' + rownumber + '-' + 'Lesson name' + ',';
return;
}
if(!row[10]){
f1=f1 + 'row ' + rownumber + '-' + 'Lesson Plan' + ',';
return;
}
if(!row[11] || isNaN(row[11])){
f1=f1 + 'row ' + rownumber + '-' + 'No of period' + ',';
return;
}
 if(isNaN(new Date(row[12]))) {
f1=f1 + 'row ' + rownumber + '-' + 'From date' + ',';
return;
}
 if(isNaN(new Date(row[13]))) {
f1=f1 + 'row ' + rownumber + '-' + 'To date' + ',';
return;
}
 if(isNaN(new Date(row[14]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Actual date' + ',';
return;
}
if(!row[15]){
f1=f1 + 'row ' + rownumber + '-' + 'Topics covered' + ',';
return;
}


            searchapi(rownumber,row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13],row[14],row[15],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      academicyear,program,programcode,course,coursecode,semester,section,module,lessonno,lesson,lessonplan,noofperiod,fromdate,todate,actualdate,topics,status <br /><br />
