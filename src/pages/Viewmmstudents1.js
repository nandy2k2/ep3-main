...
import AddUserModal from './Addmmstudents1';
import AddUserModalBulk from './Addmmstudents1bulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemstudents1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'currentyear',
headerName:'Current academic year',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmstudents1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmstudents1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmstudents1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const enabled=user.enabled;
            ...
            const response =await ep1.get('/api/v2/updatemstudents1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
