...
return;
  ...
  const searchapi = async (rownumber,year,employee,empid,role,designation,category,item,due,concession,paid,balance,paydate,payref,cashbook,type,level,status1) => {
...
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Item' + ',';
return;
}
if(!row[7] || isNaN(row[7])){
f1=f1 + 'row ' + rownumber + '-' + 'Due' + ',';
return;
}
if(!row[8] || isNaN(row[8])){
f1=f1 + 'row ' + rownumber + '-' + 'Concession' + ',';
return;
}
if(!row[9] || isNaN(row[9])){
f1=f1 + 'row ' + rownumber + '-' + 'Paid' + ',';
return;
}
if(!row[10] || isNaN(row[10])){
f1=f1 + 'row ' + rownumber + '-' + 'Balance' + ',';
return;
}
 if(isNaN(new Date(row[11]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Pay date' + ',';
return;
}
if(!row[12]){
f1=f1 + 'row ' + rownumber + '-' + 'Payref' + ',';
return;
}
if(!row[13]){
f1=f1 + 'row ' + rownumber + '-' + 'Cashbook' + ',';
return;
}
if(!row[14]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,employee,empid,role,designation,category,item,due,concession,paid,balance,paydate,payref,cashbook,type,level,status <br /><br />
