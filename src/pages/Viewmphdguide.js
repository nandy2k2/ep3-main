...
import AddUserModal from './Addmphdguide';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletephdguidebyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'yor',
headerName:'Year of registration',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'yop',
headerName:'Year of scholar award',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getphdguidebyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getphdguidecountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getphdguidesecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const researchguide=user.researchguide;
const yog=user.yog;
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updatephdguidebyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
