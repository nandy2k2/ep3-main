...
return;
  ...
  const searchapi = async (rownumber,campus,address,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createunivampusbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      campus,address,type,level,status <br /><br />
