...
import AddUserModal from './Addmpbilling';
import AddUserModalBulk from './Addmpbillingbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepbillingbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpbillingbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpbillingcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpbillingsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const billdate=new Date(user.billdate);
...
const paystatus=user.paystatus;
            ...
            const response =await ep1.get('/api/v2/updatepbillingbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
