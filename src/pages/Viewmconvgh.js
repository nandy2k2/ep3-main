...
import AddUserModal from './Addmconvgh';
import AddUserModalBulk from './Addmconvghbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteconvghbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getconvghbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getconvghcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getconvghsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const guesthouse=user.guesthouse;
const room=user.room;
const roomtype=user.roomtype;
const guest=user.guest;
const clogin=user.clogin;
...
const type=user.type;
            ...
            const response =await ep1.get('/api/v2/updateconvghbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
