...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,examdate,resultdate,noofdays,doclink,type,level,status1) => {
...
f1=f1 + 'row ' + rownumber + '-' + 'Last date of exam' + ',';
return;
}
 if(isNaN(new Date(row[4]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Result date' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'No of days' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,examdate,resultdate,noofdays,doclink,type,level,status <br /><br />
