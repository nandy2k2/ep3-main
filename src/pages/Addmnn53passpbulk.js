...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,appeared,passed,passp,doclink,type,level,status1) => {
...
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'No of appeared' + ',';
return;
}
if(!row[4] || isNaN(row[4])){
f1=f1 + 'row ' + rownumber + '-' + 'No of passed' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'Pass percentage' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,appeared,passed,passp,doclink,type,level,status <br /><br />
