...
import AddUserModal from './Addmnn244';
import AddUserModalBulk from './Addmnn244bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn244byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn244byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn244countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn244secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const mplatform=user.mplatform;
const certdate=new Date(user.certdate);
const mrole=user.mrole;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn244byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
