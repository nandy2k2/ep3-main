...
import AddUserModal from './Addmclassnew';
import AddUserModalBulk from './Addmclassnewbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteclassnewbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClickgo = async(e, row) => {
      ...
      global1.classid=row._id;
      global1.facprogram=row.program;
      global1.facprogramcode=row.programcode;
      global1.facsemester=row.semester;
      global1.facsection=row.section;
      global1.facclassdate=row.classdate;
      
      navigate('/dashmattendancenew');
...
return '';
    ...
    const fetchViewPage = async () => {
      ...
      const response = await ep1.get('/api/v2/getclassnewbyfacdate', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getclassnewcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getclassnewsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const classtime=user.classtime;
...
const classtype=user.classtype;
            ...
            const response =await ep1.get('/api/v2/updateclassnewbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
      ...
      <React.Fragment>
        ...
        <Box display="flex" marginBottom={4} marginTop={2}>
         ...
         <Button
              variant="contained"
              color="success"
              style={{ padding: '5px 10px', marginRight: '4px', fontSize: '12px', height: '40px', width: '80px' }}
