...
import AddUserModal from './Addmmsyllabus';
import AddUserModalBulk from './Addmmsyllabusbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemsyllabusbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClickm = async() => {
      ...
      const response = await epai1.get('/api/v1/getresponse2', {
          params: {
              ...
              user:user,
              colid:colid,
              prompt:'Create detailed week wise syllabus for ' + coursename + ' with focus on ' + keywords
          }
    ...
    element.download = coursename + ".html";
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmsyllabusbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmsyllabuscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmsyllabussecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
...
const courselink=user.courselink;
...
const completed=user.completed;
            ...
            const response =await ep1.get('/api/v2/updatemsyllabusbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
