...
return;
  ...
  const searchapi = async (rownumber,year,designation,sanctioned,employed,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn25byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
f1=f1 + 'row ' + rownumber + '-' + 'Sanctioned Post' + ',';
return;
}
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'No of faculties working for three years' + ',';
return;
}
if(!row[4]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,designation,sanctioned,employed,type,level,status <br /><br />
