...
import AddUserModal from './Addmtbcolumnsall';
import AddUserModalBulk from './Addmtbcolumnsallbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletetbcolumnsallbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gettbcolumnsallbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gettbcolumnsallcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gettbcolumnsallsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const fields=user.fields;
            ...
            const response =await ep1.get('/api/v2/updatetbcolumnsallbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
