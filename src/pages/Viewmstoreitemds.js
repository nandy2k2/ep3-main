...
import AddUserModal from './Addmstoreitemds';
import AddUserModalBulk from './Addmstoreitemdsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletestoreitemdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getstoreitemdsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getstoreitemdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getstoreitemdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const status=user.status;
            ...
            const response =await ep1.get('/api/v2/updatestoreitemdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
