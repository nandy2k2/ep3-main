...
return;
  ...
  const searchapi = async (rownumber,testid,section,description,noofquestions,imagelink,doclink,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createmisections1byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'No of questions' + ',';
return;
}
if(!row[4]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      testid,section,description,noofquestions,imagelink,doclink,type,level,status <br /><br />
