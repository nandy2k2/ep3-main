...
import AddUserModal from './Addmpdischarge';
import AddUserModalBulk from './Addmpdischargebulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepdischargebyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpdischargebyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpdischargecountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpdischargesecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const discharge=user.discharge;
...
const followup=user.followup;
            ...
            const response =await ep1.get('/api/v2/updatepdischargebyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
