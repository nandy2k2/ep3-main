...
return;
  ...
  const searchapi = async (rownumber,faculty,designation,year,sdate,salary,allowance,ugc,doclink,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn22byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
f1=f1 + 'row ' + rownumber + '-' + 'Salary date' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      faculty,designation,year,sdate,salary,allowance,ugc,doclink,type,level,status <br /><br />
