...
export default function RelativeGradingConfigurationPage() {
  ...
  const loadOptions = async (optionFilters = {}) => {
    try {
      ...
      const res = await ep1.get("/api/v2/relativegradingconfiguration/options", { params });
      setOptions({
  ...
  const loadRows = async (nextFilters = filters) => {
    try {
      ...
      const res = await ep1.get("/api/v2/relativegradingconfiguration", { params });
      setRows(res.data.data || []);
    } catch (err) {
      setError(err.response?.data?.message || "Unable to load relative grading configuration");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveRow = async (event) => {
    ...
    try {
      ...
      if (editingId) {
        await ep1.post("/api/v2/relativegradingconfiguration/update", { ...payload, id: editingId });
        setMessage("Relative grading configuration updated");
      } else {
        await ep1.post("/api/v2/relativegradingconfiguration", payload);
        setMessage("Relative grading configuration created");
      }
      resetForm();
      await loadRows();
      await loadOptions();
    } catch (err) {
      setError(err.response?.data?.message || "Unable to save relative grading configuration");
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteRow = async (row) => {
    if (!window.confirm(`Delete relative grade ${row.grade}?`)) return;
    try {
      setLoading(true);
      await ep1.post("/api/v2/relativegradingconfiguration/delete", { id: row._id, colid });
      setMessage("Relative grading configuration deleted");
      await loadRows();
      await loadOptions();
    } catch (err) {
      setError(err.response?.data?.message || "Unable to delete relative grading configuration");
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    { field: "from", headerName: "From", width: 110, type: "number" },
    { field: "to", headerName: "To", width: 110, type: "number" },
  ...
  return (
    ...
    <Box p={3}>
      <Stack direction={{ xs: "column", md: "row" }} spacing={2} justifyContent="space-between" sx={{ mb: 2 }}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Relative Grading Configuration</Typography>
          <Typography variant="body2" color="text.secondary">Configure relative grade bands course-wise.</Typography>
        </Box>
      ...
      <Paper component="form" onSubmit={saveRow} sx={{ p: 2, mb: 2 }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={2}>
            <TextField fullWidth type="number" label="From" value={form.from} onChange={(e) => updateForm("from", e.target.value)} required />
          </Grid>
          <Grid item xs={12} md={2}>
            <TextField fullWidth type="number" label="To" value={form.to} onChange={(e) => updateForm("to", e.target.value)} required />
          </Grid>
          <Grid item xs={12} md={2}>
      ...
      <Paper sx={{ p: 1, overflowX: "auto" }}>
        <DataGrid
          ...
          slotProps={{ toolbar: { showQuickFilter: true, csvOptions: { fileName: "relative_grading_configuration" } } }}
