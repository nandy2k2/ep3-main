...
const levelref=useRef();
    ...
    const {
      ...
      const setfeebook=(id)=> {
if(transcript) {
feebookref.current.value=transcript;
 }
 }
...
const feebook=feebookref.current.value;
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createfeebookbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
status1:'Submitted',
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      <p>{transcript}</p>
       ...
       <button onClick={setfeebook}>Set Fee book</button>
 ...
 <button onClick={setlevel}>Set Level</button>
    ...
    <p>Fee book</p>
<TextField id="outlined-basic"  type="text" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={feebookref} /><br /><br />
