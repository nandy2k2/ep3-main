...
import AddUserModal from './Addmmassets';
import AddUserModalBulk from './Addmmassetsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemassetsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClicksy = async(e, row) => {
      ...
      //do whatever you want with the row
      //alert(row._id);
      global1.assetid=row._id;
      global1.asset=row.asset;
      //global1.faccoursecode=row.coursecode;
      navigate('/dashmmassetassign');
    };

    const columns = [
       { field: '_id', headerName: 'Asset ID', width: 260 },
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmassetsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmassetscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmassetssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const assettype=user.assettype;
...
const poid=user.poid;
            ...
            const response =await ep1.get('/api/v2/updatemassetsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
