...
return;
  ...
  const searchapi = async (rownumber,patient,puser,admid,allergy,duration,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createpallergiesbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      patient,puser,admid,allergy,duration,status <br /><br />
