...
import AddUserModal from './Addmmstudents';
import AddUserModalBulk from './Addmmstudentsbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemstudentsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmstudentsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmstudentscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmstudentssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const enabled=user.enabled;
            ...
            const response =await ep1.get('/api/v2/updatemstudentsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
