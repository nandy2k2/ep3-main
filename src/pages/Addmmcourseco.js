...
const cocoderef=useRef();
const coref=useRef();
const typeref=useRef();
const targetlevelref=useRef();
...
const setcocode=(id)=> {
if(transcript) {
cocoderef.current.value=transcript;
 }
 }
const setco=(id)=> {
if(transcript) {
...
// const coursecode=coursecoderef.current.value;
const cocode=cocoderef.current.value;
const co=coref.current.value;
const type=typeref.current.value;
const targetlevel=targetlevelref.current.value;
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createmcoursecobyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
status1:'Submitted',
 ...
 <button onClick={setcocode}>Set CO code</button>
 <button onClick={setco}>Set Course outcome</button>
...
<p>CO code</p>
<TextField id="outlined-basic"  type="text" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={cocoderef} /><br /><br />
...
<InputLabel id="targetlevel">Target level</InputLabel><Select labelId="targetlevel"
id="targetlevel"
inputRef={targetlevelref}
sx={{ width: '100%'}}
>
