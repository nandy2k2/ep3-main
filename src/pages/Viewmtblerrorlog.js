...
import AddUserModal from './Addmtblerrorlog';
import AddUserModalBulk from './Addmtblerrorlogbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletetblerrorlogbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gettblerrorlogbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gettblerrorlogcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gettblerrorlogsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const pagename=user.pagename;
const error=user.error;
const errordate=new Date(user.errordate);
const type=user.type;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatetblerrorlogbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
