...
const MenuCasAdminFullTree = ({ open }) => {
  return (
...
{open && <ListItemText primary="Exam Seat allotment" />}
 ...
 <ListItem button component={RouterLink} to="/dashmexamnewrubrics1admin">
<ListItemIcon>
<PersonIcon />
</ListItemIcon>
{open && <ListItemText primaryTypographyProps={{fontSize: "14px"}} primary="Internal marks entry" />}
</ListItem>
...
const MenuCasAdminFilteredTree = ({ open }) => {
  ...
  if (normalizedRole === 'all' || normalizedRole === 'admin') {
    return <MenuCasAdminFullTree open={open} />;
  }
...
export function mainListItems({ open }) {
  return <MenuCasAdminFilteredTree open={open} />;
}
