...
return;
  ...
  const searchapi = async (rownumber,domain,username,pagename,error,errordate,type,status1) => {
...
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Page name' + ',';
return;
}
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Error' + ',';
return;
}
 if(isNaN(new Date(row[4]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Error date' + ',';
return;
}
if(!row[5]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      domain,username,pagename,error,errordate,type,status <br /><br />
