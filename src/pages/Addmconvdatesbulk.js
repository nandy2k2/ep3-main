...
return;
  ...
  const searchapi = async (rownumber,year,event,eventdate,description,location,reglink,type,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createconvdatesbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
f1=f1 + 'row ' + rownumber + '-' + 'Event date' + ',';
return;
}
if(!row[3]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Registration link' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,event,eventdate,description,location,reglink,type,status <br /><br />
