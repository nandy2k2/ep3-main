...
import AddUserModal from './AddmUser';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteUserbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/Userdocs', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
        const response = await ep1.get('/api/v2/getUsercount', {
          params: {
            token: token,
            colid: colid,
            user: user
      ...
      const getgraphdatasecond = async () => {
        const response = await ep1.get('/api/v2/getUsersecond', {
          params: {
            token: token,
            colid: colid,
            user: user
...
const Department=user.Department;
            ...
            const response =await ep1.get('/api/v2/updateUserbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
