...
return;
  ...
  const searchapi = async (rownumber,committee,comcode,year,university,faculty,isphd,rdate,designation,salary,ugc,industryexp,acadexpyears,doclink,type,level,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Selection Committee' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Committee code' + ',';
return;
}
if(!row[2]){
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'University' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Faculty name' + ',';
return;
}
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Is PhD' + ',';
return;
}
 if(isNaN(new Date(row[6]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Recruitment date' + ',';
return;
}
if(!row[7]){
...
f1=f1 + 'row ' + rownumber + '-' + 'Salary' + ',';
return;
}
if(!row[9]){
f1=f1 + 'row ' + rownumber + '-' + 'Is UGC Scale' + ',';
return;
}
if(!row[10] || isNaN(row[10])){
f1=f1 + 'row ' + rownumber + '-' + 'Industry experience in years' + ',';
return;
}
if(!row[11] || isNaN(row[11])){
f1=f1 + 'row ' + rownumber + '-' + 'Academic experience in years' + ',';
return;
}
if(!row[12]){
f1=f1 + 'row ' + rownumber + '-' + 'Document link' + ',';
return;
}
if(!row[13]){
...
if(!row[14]){
f1=f1 + 'row ' + rownumber + '-' + 'Level' + ',';
return;
}


            searchapi(rownumber,row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13],row[14],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      committee,comcode,year,university,faculty,isphd,rdate,designation,salary,ugc,industryexp,acadexpyears,doclink,type,level,status <br /><br />
