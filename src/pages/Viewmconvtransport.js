...
import AddUserModal from './Addmconvtransport';
import AddUserModalBulk from './Addmconvtransportbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteconvtransportbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getconvtransportbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getconvtransportcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getconvtransportsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const type=user.type;
            ...
            const response =await ep1.get('/api/v2/updateconvtransportbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
