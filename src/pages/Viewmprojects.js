...
import AddUserModal from './Addmprojects';
  ...
  const [open, setOpen] = React.useState(false);
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteprojectsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getprojectsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getprojectscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getprojectssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const duration=user.duration;
            ...
            const response =await ep1.get('/api/v2/updateprojectsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
duration:duration,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.name + '~' + row.agency + '~' + row.project;
