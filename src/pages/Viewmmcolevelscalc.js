...
import AddUserModal from './Addmmcolevels';
import AddUserModalBulk from './Addmmcolevelsbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemcolevelsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const getgraphdatasecond1 = async () => {
        const coursename1=global1.faccoursename;
        const coursecode1=global1.faccoursecode;
        const response = await ep1.get('/api/v2/getmattcalcsecondbyfac', {
          params: {
            token: token,
            colid: colid,
            user: user,
    ...
    const getgraphdata = async () => {
      const coursename=global1.faccoursename;
      const coursecode=global1.faccoursecode;
      const response = await ep1.get('/api/v2/getmcolevelscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
...
const minvalue=user.minvalue;
const maxvalue=user.maxvalue;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatemcolevelsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
maxvalue:maxvalue,
      ...
      const getlevels=(co,val)=> {
        for (var i=0;i<rows.length;i++) {
            //alert(rows[i].co);
            if(rows[i].co==co) {
                var minval=rows[i].minvalue;
                var maxval=rows[i].maxvalue;
    ...
    {third.map((meetup3) => {
                return (
                    <tr>
                        ...
                        <td>
                            {getlevels(meetup3._id,meetup3.total_attendance)}
                        </td>
