...
const setapplylink=(id)=> {
if(transcript) {
applylinkref.current.value=transcript;
 }
 }
const setlevel=(id)=> {
if(transcript) {
...
const applylink=applylinkref.current.value;
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createwebcoursesbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
status1:'Submitted',
 ...
 <button onClick={setapplylink}>Set Apply link</button>
...
<p>Apply link</p>
<TextField id="outlined-basic"  type="text" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={applylinkref} /><br /><br />
