...
import AddUserModal from './Addmnn15';
import AddUserModalBulk from './Addmnn15bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn15byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn15byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn15countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn15secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const moocp=user.moocp;
const moocper=user.moocper;
const moocno=user.moocno;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn15byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
