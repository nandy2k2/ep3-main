...
return;
  ...
  const searchapi = async (rownumber,transactionid,orderid,bankref,paymode,bank,amount,username,regno,clientcolid,sessionslot,testc,type,paydate,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Transaction id' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Order id' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Bank ref' + ',';
return;
}
if(!row[3]){
...
if(!row[8]){
f1=f1 + 'row ' + rownumber + '-' + 'Client colid' + ',';
return;
}
if(!row[9]){
f1=f1 + 'row ' + rownumber + '-' + 'Session slot' + ',';
return;
}
if(!row[10]){
f1=f1 + 'row ' + rownumber + '-' + 'Test' + ',';
return;
}
if(!row[11]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      transactionid,orderid,bankref,paymode,bank,amount,username,regno,clientcolid,sessionslot,testc,type,paydate,status <br /><br />
