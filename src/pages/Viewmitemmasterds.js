...
import AddUserModal from './Addmitemmasterds';
import AddUserModalBulk from './Addmitemmasterdsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteitemmasterdsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getitemmasterdsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getitemmasterdscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getitemmasterdssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const status=user.status;
            ...
            const response =await ep1.get('/api/v2/updateitemmasterdsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
