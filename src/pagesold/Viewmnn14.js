...
import AddUserModal from './Addmnn14';
import AddUserModalBulk from './Addmnn14bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn14byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'studproj',
headerName:'No of students participating in projects',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'studintern',
headerName:'No of students in internship',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn14byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn14countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn14secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const internship=user.internship;
const studproj=user.studproj;
const studintern=user.studintern;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn14byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
level:level,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.program + '~' + row.programcode;
