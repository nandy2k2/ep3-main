...
return;
  ...
  const searchapi = async (rownumber,patient,puser,admid,treatment,duration,followup,startdate,status1) => {
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Treatment plan' + ',';
return;
}
if(!row[4]){
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Follow up' + ',';
return;
}
 if(isNaN(new Date(row[6]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Start up date' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      patient,puser,admid,treatment,duration,followup,startdate,status <br /><br />
