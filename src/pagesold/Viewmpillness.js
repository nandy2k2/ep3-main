...
import AddUserModal from './Addmpillness';
import AddUserModalBulk from './Addmpillnessbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepillnessbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpillnessbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpillnesscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpillnesssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const doccomments=user.doccomments;
            ...
            const response =await ep1.get('/api/v2/updatepillnessbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
