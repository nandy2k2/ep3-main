...
import AddUserModal from './Addmpcounselc';
import AddUserModalBulk from './Addmpcounselcbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepcounselcbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpcounselcbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpcounselccountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpcounselcsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const pdate=new Date(user.pdate);
const discussion=user.discussion;
const ptime=user.ptime;
const doccomments=user.doccomments;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatepcounselcbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
