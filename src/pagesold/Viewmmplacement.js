...
import AddUserModal from './Addmmplacement';
import AddUserModalBulk from './Addmmplacementbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemplacementbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmplacementbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmplacementcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmplacementsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const empaddress=user.empaddress;
const empcontact=user.empcontact;
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updatemplacementbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
