...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,student,regno,exam,sector,result,doclink,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn84byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Competitive exam' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,student,regno,exam,sector,result,doclink,type,level,status <br /><br />
