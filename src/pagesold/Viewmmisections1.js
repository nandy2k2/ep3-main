...
import AddUserModal from './Addmmisections1';
import AddUserModalBulk from './Addmmisections1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemisections1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
     ...
     const onButtonClickq = async(e, row) => {
          e.stopPropagation();
          global1.isectionid=row._id;
          global1.isection =row.section;
          
          
          navigate('/dashmmiqnew');
        };
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmisections1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmisections1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmisections1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatemisections1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
