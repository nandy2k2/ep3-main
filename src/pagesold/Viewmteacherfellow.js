...
import AddUserModal from './Addmteacherfellow';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteteacherfellowbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getteacherfellowbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getteacherfellowcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getteacherfellowsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updateteacherfellowbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
