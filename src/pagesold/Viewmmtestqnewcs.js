...
import AddUserModal from './Addmmtestqnewcs';
import AddUserModalBulk from './Addmmtestqnewcsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtestqnewcsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmtestqnewcsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtestqnewcscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtestqnewcssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const questionimg=user.questionimg;
const questionlink=user.questionlink;
const optiona=user.optiona;
const optionb=user.optionb;
const optionc=user.optionc;
const optiond=user.optiond;
const correcta=user.correcta;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatemtestqnewcsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
