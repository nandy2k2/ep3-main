...
import AddUserModal from './Addmicu';
import AddUserModalBulk from './Addmicubulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteicubyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    //do whatever you want with the row
    //alert(row._id);
    global1.icucode=row.title;
    alert('Selected ICU ' + global1.icucode);
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/geticubyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/geticucountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/geticusecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const capacity=user.capacity;
            ...
            const response =await ep1.get('/api/v2/updateicubyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
