...
import AddUserModal from './Addmncas241';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v1/deletencas241byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'type',
headerName:'Type of Research guidance',
type:'dropdown',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v1/getncas241byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v1/getncas241countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v1/getncas241secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const score=user.score;
            ...
            const response =await ep1.get('/api/v1/updatencas241byfac', {
            params: {
            id: user._id,
            user: user,
            token:token,
