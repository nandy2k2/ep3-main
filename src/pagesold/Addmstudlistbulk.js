...
return;
  ...
  const searchapi = async (rownumber,year,department,program,programcode,students,gender,category,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createstudlistbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
f1=f1 + 'row ' + rownumber + '-' + 'No of students' + ',';
return;
}
if(!row[5]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,department,program,programcode,students,gender,category,type,level,status <br /><br />
