...
import AddUserModal from './Addmpcounselnew';
import AddUserModalBulk from './Addmpcounselnewbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepcounselnewbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpcounselnewbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpcounselnewcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpcounselnewsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const illness=user.illness;
const treatment=user.treatment;
const surgery=user.surgery;
const amount=user.amount;
const mdate=new Date(user.mdate);
const followupdate=new Date(user.followupdate);
const doctorref=user.doctorref;
const doctor=user.doctor;
const assignedto=user.assignedto;
const leadstatus=user.leadstatus;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatepcounselnewbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
