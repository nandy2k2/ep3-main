...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,course,lms,students,faculty,doclink,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn56byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Is LMS' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'No of students using' + ',';
return;
}
if(!row[6] || isNaN(row[6])){
f1=f1 + 'row ' + rownumber + '-' + 'No of faculties using' + ',';
return;
}
if(!row[7]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,course,lms,students,faculty,doclink,type,level,status <br /><br />
