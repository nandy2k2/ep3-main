...
import AddUserModal from './Addmncas11';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v1/deletencas11byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'noofclass',
headerName:'Number of classes taught',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'totalclass',
headerName:'Total classes assigned',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'percentage',
headerName:'Percentage of classes taught',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v1/getncas11byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v1/getncas11countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v1/getncas11secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const noofclass=user.noofclass;
const totalclass=user.totalclass;
...
const score=user.score;
            ...
            const response =await ep1.get('/api/v1/updatencas11byfac', {
            params: {
            id: user._id,
            user: user,
            token:token,
...
score:score,
    ...
    return (
      <React.Fragment>
        ...
        <Box draggable display="flex" marginBottom={4} marginTop={2}>
