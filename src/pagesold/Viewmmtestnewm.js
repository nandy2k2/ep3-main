...
import AddUserModal from './Addmmtestnewm';
import AddUserModalBulk from './Addmmtestnewmbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtestnewmbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClicksessions = async(e, row) => {
      e.stopPropagation();
      global1.testid=row._id;
      global1.testname=row.testc;
      global1.testcoursecode=row.coursecode;
      
      navigate('/dashmmtestsessions');
    };
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmtestnewmbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtestnewmcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtestnewmsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatemtestnewmbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
