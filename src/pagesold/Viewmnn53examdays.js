...
import AddUserModal from './Addmnn53examdays';
import AddUserModalBulk from './Addmnn53examdaysbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn53examdaysbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn53examdaysbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn53examdayscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn53examdayssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const resultdate=new Date(user.resultdate);
const noofdays=user.noofdays;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn53examdaysbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
