...
import AddUserModal from './Addmmvendors';
import AddUserModalBulk from './Addmmvendorsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemvendorsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmvendorsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmvendorscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmvendorssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updatemvendorsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
