...
return;
  ...
  const searchapi = async (rownumber,program,programcode,year,students,faculty,alumni,parents,employer,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn12byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Students feedback' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Faculty feedback' + ',';
return;
}
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Alumni feedback' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Parents feedback' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Employer feedback' + ',';
return;
}
if(!row[8]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      program,programcode,year,students,faculty,alumni,parents,employer,type,level,status <br /><br />
