...
import AddUserModal from './Addmnn23';
import AddUserModalBulk from './Addmnn23bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn23byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn23byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn23countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn23secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const state1=user.state1;
const state2=user.state2;
const country=user.country;
const foreigner=user.foreigner;
const category1=user.category1;
const category2=user.category2;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn23byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
level:level,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.faculty + '~' + row.designation;
