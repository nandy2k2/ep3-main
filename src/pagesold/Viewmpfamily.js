...
import AddUserModal from './Addmpfamily';
import AddUserModalBulk from './Addmpfamilybulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepfamilybyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpfamilybyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpfamilycountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpfamilysecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const relation=user.relation;
            ...
            const response =await ep1.get('/api/v2/updatepfamilybyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
