...
return;
  ...
  const searchapi = async (rownumber,patient,puser,admid,lab,test,result,finding,testdate,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createpimagingbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Findings' + ',';
return;
}
 if(isNaN(new Date(row[7]))) {
f1=f1 + 'row ' + rownumber + '-' + 'test date' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      patient,puser,admid,lab,test,result,finding,testdate,status <br /><br />
