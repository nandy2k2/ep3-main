...
import AddUserModal from './Addmmguides';
import AddUserModalBulk from './Addmmguidesbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemguidesbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmguidesbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmguidescountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmguidessecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const link=user.link;
            ...
            const response =await ep1.get('/api/v2/updatemguidesbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
