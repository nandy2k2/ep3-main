...
import AddUserModal from './Addmexammarksall';
import AddUserModalBulk from './Addmexammarksallbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteexammarksallbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'eafull',
headerName:'External full marks',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'eamarks',
headerName:'External marks obtained',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getexammarksallbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getexammarksallcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getexammarksallsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const iafull=user.iafull;
const iamarks=user.iamarks;
const eafull=user.eafull;
const eamarks=user.eamarks;
const totalfull=user.totalfull;
const totalmarks=user.totalmarks;
const totalp=user.totalp;
const egrade=user.egrade;
const semester=user.semester;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updateexammarksallbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
