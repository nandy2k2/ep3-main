...
import AddUserModal from './Addmbooksadmin';
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/bookdocs', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
        const response = await ep1.get('/api/v2/getbookcount', {
          params: {
            token: token,
            colid: colid,
            user: user
      ...
      const getgraphdatasecond = async () => {
        const response = await ep1.get('/api/v2/getbooksecond', {
          params: {
            token: token,
            colid: colid,
            user: user
