...
return;
  ...
  const searchapi = async (rownumber,wday,period,starttime,endtime,program,semester,section,faculty,facultyid,course,slotstatus,room,capacity,type,level,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Day of week' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Period' + ',';
return;
}
 if(isNaN(new Date(row[2]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Start time' + ',';
return;
}
 if(isNaN(new Date(row[3]))) {
f1=f1 + 'row ' + rownumber + '-' + 'End time' + ',';
return;
}
if(!row[4]){
...
if(!row[10]){
f1=f1 + 'row ' + rownumber + '-' + 'Slot status' + ',';
return;
}
if(!row[11]){
f1=f1 + 'row ' + rownumber + '-' + 'Room' + ',';
return;
}
if(!row[13]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      wday,period,starttime,endtime,program,semester,section,faculty,facultyid,course,slotstatus,room,capacity,type,level,status <br /><br />
