...
import AddUserModal from './Addmmbalancesheet';
import AddUserModalBulk from './Addmmbalancesheetbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletembalancesheetbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmbalancesheetbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmbalancesheetcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmbalancesheetsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const credit=user.credit;
            ...
            const response =await ep1.get('/api/v2/updatembalancesheetbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
