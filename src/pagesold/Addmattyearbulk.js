...
return;
  ...
  const searchapi = async (rownumber,year,coursecode,co,attained,total,pattained,level1,level2,level3,target,fleveln,flevel,doclink,status1) => {
...
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'CO' + ',';
return;
}
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'Attained no' + ',';
return;
}
if(!row[4] || isNaN(row[4])){
f1=f1 + 'row ' + rownumber + '-' + 'Total students' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'Percentage attained' + ',';
return;
}
if(!row[6] || isNaN(row[6])){
f1=f1 + 'row ' + rownumber + '-' + 'Level 1' + ',';
return;
}
if(!row[7] || isNaN(row[7])){
f1=f1 + 'row ' + rownumber + '-' + 'Level 2' + ',';
return;
}
if(!row[8] || isNaN(row[8])){
f1=f1 + 'row ' + rownumber + '-' + 'Level 3' + ',';
return;
}
if(!row[9] || isNaN(row[9])){
f1=f1 + 'row ' + rownumber + '-' + 'Threshold Target' + ',';
return;
}
if(!row[10] || isNaN(row[10])){
f1=f1 + 'row ' + rownumber + '-' + 'Level Number' + ',';
return;
}
if(!row[11]){
...
return;
            ...
            searchapi(rownumber,row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],status1);
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,coursecode,co,attained,total,pattained,level1,level2,level3,target,fleveln,flevel,doclink,status <br /><br />
