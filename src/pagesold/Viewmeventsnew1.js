...
const onButtonClickc = async(e, row) => {
  e.stopPropagation();
  ...
  const response = await epai1.get('/api/v1/getresponse2', {
      params: {
          ...
          user: user,
          colid: colid,
          prompt:'Create a  circular for event ' + row.event + ' with details such as ' + row.description + ' with circular date one week before ' + formatdates(row.startdate)
      }

  });
  var backend= '<html><head><title>' + row.event + '</title></head><body>'; 
  backend=backend + '<br /><br />';
  backend=backend + '<h5>' + 'Circular' +  '</h5><hr />';
  //alert(response.data.data.classes);
  //const a=response.data.data.classes;
  const aiarray=response.data.data.classes.split('\n');
  //console.log('Count ' + aiarray.length);
...
const file = new Blob([backend], {type: 'text/plain'});
element.href = URL.createObjectURL(file);
element.download = 'circular_' + row.event + ".html";
document.body.appendChild(element); // Required for this to work in FireFox
element.click();
  //const a = await fetchViewPage();
};
...
return '';
          ...
          { field: 'actions', headerName: 'Actions', width: 700, renderCell: (params) => {
            return (
             <table>
                <tr>
                  <td>
              ...
              >
                AI brochure
                  ...
                  <td>
                  <Button
                onClick={(e) => onButtonClickc(e, params.row)}
                variant="contained"
              >
                AI circular
              </Button>
...
duration:duration,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.event + '~' + row.year + '~' + row.coordinator;
