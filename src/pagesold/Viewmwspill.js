...
import AddUserModal from './Addmwspill';
import AddUserModalBulk from './Addmwspillbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletewspillbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getwspillbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getwspillcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getwspillsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const status=user.status;
            ...
            const response =await ep1.get('/api/v2/updatewspillbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
