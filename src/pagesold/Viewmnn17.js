...
import AddUserModal from './Addmnn17';
import AddUserModalBulk from './Addmnn17bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn17byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'revisionp',
headerName:'Revision percentage',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn17byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn17countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn17secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const revisionp=user.revisionp;
const bosdate=new Date(user.bosdate);
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn17byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
