...
import AddUserModal from './Addmmjournal2';
import AddUserModalBulk from './Addmmjournal2bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemjournal2byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmjournal2byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmjournal2countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmjournal2secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const subledger=user.subledger;
const cogs=user.cogs;
const activitydate=new Date(user.activitydate);
...
const empid=user.empid;
            ...
            const response =await ep1.get('/api/v2/updatemjournal2byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
