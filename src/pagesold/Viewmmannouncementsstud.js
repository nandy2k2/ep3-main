...
import AddUserModal from './Addmmanouncements';
import AddUserModalBulk from './Addmmanouncementsbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemanouncementsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmanouncementsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmanouncementscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmanouncementssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
...
const target=user.target;
            ...
            const response =await ep1.get('/api/v2/updatemanouncementsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
