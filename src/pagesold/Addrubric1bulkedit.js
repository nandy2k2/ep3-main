...
return;
  ...
  const searchapi = async (rownumber,program,semester,section,faculty,facultyid,course,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createworkloadnbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
const formattedData = ({
      ...
      exam: row[8],
      examcode: row[9],
      internalexamname: row[10],
          internalfull: parseInt(row[11]),
          internalobtainmark: parseInt(row[12]),
    //   internalmarks: [
    //     {
    //       internalexamname: row[10],
    //       internalfull: parseInt(row[11]),
    ...
    //       attendancefull: row.attendancefull,
    //       attendanceobtainmark: row.attendanceobtainmark,
    //     },
    //   ],
    //   internshipmarks: [
    //     {
    //       internshipname: row.internshipname,
    //       internshipfull: row.internshipfull,
    //       internshipobtainmark: row.internshipobtainmark,
    //     },
    //   ],
    //   extracurriculummarks: [
    //     {
    //       extracurriculumname: row.extracurriculumname,
    //       extracurriculumfull: row.extracurriculumfull,
    //       extracurriculumobtainmark: row.extracurriculumobtainmark,
    //     },
    //   ],
    //   externalfull: row.externalfull,
    //   externalmarks: row.externalmarks,
    })

  
        const a1=await ep1.post("/api/v2/createrubricds1", formattedData );



            //rownumber=rownumber + 1;
//             if(!row[0]){
// f1=f1 + 'row ' + rownumber + '-' + 'undefined' + ',';
// return;
// }
// if(!row[1]){
// f1=f1 + 'row ' + rownumber + '-' + 'undefined' + ',';
...
const handleClose=()=> {
  ...
  return (
    ...
    >
      ...
      student,regno,program,programcode,year,semester,course,coursecode,exam,examcode,ia name,ia full marks,ia marks obtained,status <br /><br />
