...
const cdayref=useRef();
const ctimeref=useRef();
...
const setctime=(id)=> {
if(transcript) {
ctimeref.current.value=transcript;
 }
 }
const setvendor=(id)=> {
if(transcript) {
...
const cday=cdayref.current.value;
const ctime=ctimeref.current.value;
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createwcolschedule1byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
status1:'Submitted',
 ...
 <button onClick={setctime}>Set Collection time</button>
...
<p>Collection day</p>
<TextField id="outlined-basic"  type="String" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={cdayref} /><br /><br />

<p>Collection time</p>
<TextField id="outlined-basic"  type="text" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={ctimeref} /><br /><br />
