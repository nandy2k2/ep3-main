...
import AddUserModal from './Addmnallaccrans';
import AddUserModalBulk from './Addmnallaccransbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenallaccransbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      ...
      const response = await ep1.get('/api/v2/getnallaccransbyfac1', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnallaccranscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnallaccranssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenallaccransbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
  ...
  <Backdrop
      ...
      {rows.map((meetup3) => {
                      return (
                        <div>
                          <Paper elevation={5} sx={{ p: 2, display: 'flex', flexDirection: 'column', width: '100%' }}>
                          Question no {meetup3.qno}<br /><br />
                          Question: {meetup3.question}<br /><br />
                          Answer: <br /><br />
                          {meetup3.answer}<br /><br />
