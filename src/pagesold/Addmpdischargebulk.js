...
return;
  ...
  const searchapi = async (rownumber,patient,puser,admid,discharge,dischargedate,followup,status1) => {
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Discharge summary' + ',';
return;
}
 if(isNaN(new Date(row[4]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Discharge date' + ',';
return;
}
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Follow up plan' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      patient,puser,admid,discharge,dischargedate,followup,status <br /><br />
