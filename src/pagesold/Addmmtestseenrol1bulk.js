...
return;
  ...
  const searchapi = async (rownumber,testid,sessionid,test,sessionslot,student,regno,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createmtestseenrol1byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Session id' + ',';
return;
}
if(!row[2]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      testid,sessionid,test,sessionslot,student,regno,type,level,status <br /><br />
