...
import AddUserModal from './Addmnn53passp';
import AddUserModalBulk from './Addmnn53passpbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn53passpbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn53passpbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn53passpcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn53passpsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const appeared=user.appeared;
const passed=user.passed;
const passp=user.passp;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn53passpbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
