...
import AddUserModal from './Addmnn16';
import AddUserModalBulk from './Addmnn16bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn16byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn16byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn16countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn16secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const mooc=user.mooc;
const guestl=user.guestl;
const onlinec=user.onlinec;
const offlinec=user.offlinec;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn16byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
