...
return;
  ...
  const searchapi = async (rownumber,year,event,type,student,regno,award,amount,doclink,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createstudawardsnewstudbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Award received' + ',';
return;
}
if(!row[6] || isNaN(row[6])){
f1=f1 + 'row ' + rownumber + '-' + 'Prize amount' + ',';
return;
}
if(!row[7]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,event,type,student,regno,award,amount,doclink,status <br /><br />
