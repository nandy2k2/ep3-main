...
import AddUserModal from './Addmserbplan';
import AddUserModalBulk from './Addmserbplanbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteserbplanbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getserbplanbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getserbplancountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getserbplansecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const s6=user.s6;
            ...
            const response =await ep1.get('/api/v2/updateserbplanbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
s6:s6,
  ...
  const getcell=(celltext)=> {
    if(celltext=='Yes') {
      return (
      <div style={{width: 100, backgroundColor: '#D1C1BE', height: 100}}>
