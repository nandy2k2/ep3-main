...
import AddUserModal from './Addmseminaradmin';
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/seminardocs', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
        const response = await ep1.get('/api/v2/getseminarcount', {
          params: {
            token: token,
            colid: colid,
            user: user
      ...
      const getgraphdatasecond = async () => {
        const response = await ep1.get('/api/v2/getseminarsecond', {
          params: {
            token: token,
            colid: colid,
            user: user
