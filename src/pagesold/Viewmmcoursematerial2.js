...
function ViewPage() {
  const navigate = useNavigate();
    ...
    //           user: user,
    //           colid: colid,
    //           prompt:'Create a detailed tutorial on ' + row.description + ' with focus on ' + keywords
    //       }
    ...
    const response = await ep1.get('/api/v2/testgemini', {
          params: {
              ...
              colid: colid,
              apikey: apikey,
              question:'Create a detailed tutorial on ' + row.description + ' with focus on ' + keywords
          }
...
return '';
    ...
    const onButtonClicka = async(e, row) => {
      ...
      if(!apikey || !keywords) {
       ...
       const question='Create an assignment with details on ' + row.description + ' for ' + coursename + ' for advanced students with focus on ' + keywords + '. The assignment should be suitable for ' + coursecode + ' and should be for self-study. Provide the assignment in detail with title and description.';
      setOpen(true);
    ...
    const onButtonClickg= async () => {
      ...
      const question='Create a detailed course module plan with ' + modules + ' modules for ' + coursename + ' with focus on ' + keywords + ' and employability . Each module should have 2 to 3 topics.';
      setOpen(true);
