...
import AddUserModal from './Addmnn11';
import AddUserModalBulk from './Addmnn11bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn11byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn11byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn11countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn11secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const cbcs=user.cbcs;
const cbcsyear=user.cbcsyear;
const obe=user.obe;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn11byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
