...
import AddUserModal from './Addmpfood';
import AddUserModalBulk from './Addmpfoodbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepfoodbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpfoodbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpfoodcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpfoodsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const foodtype=user.foodtype;
const amount=user.amount;
const fooddate=new Date(user.fooddate);
const foodstatus=user.foodstatus;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatepfoodbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
