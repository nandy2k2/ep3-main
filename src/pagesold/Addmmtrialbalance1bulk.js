...
return;
  ...
  const searchapi = async (rownumber,year,accgroup,account,acctype,cogs,amount,type,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createmtrialbalance1byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,accgroup,account,acctype,cogs,amount,type,status <br /><br />
