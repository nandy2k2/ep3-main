...
function ViewPage() {
  ...
  const handleImageUpload = (event) => {
    ...
    const onButtonClicks = async(e, row) => {
      e.stopPropagation();
      global1.testid=row._id;
      
      navigate('/dashmtestscoreall');
    };
...
return '';
          ...
          { field: 'actions', headerName: 'Actions', width: 450, renderCell: (params) => {
            return (
             <table>
                <tr>
                  <td>
              ...
              >
                Questions
                  ...
                  <td>
                  <Button
                onClick={(e) => onButtonClicks(e, params.row)}
                variant="contained"
              >
                Score
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gettestnewbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
