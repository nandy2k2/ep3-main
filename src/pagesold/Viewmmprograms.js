...
import AddUserModal from './Addmmprograms';
import AddUserModalBulk from './Addmmprogramsbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemprogramsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmprogramsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmprogramscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmprogramssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const type=user.type;
            ...
            const response =await ep1.get('/api/v2/updatemprogramsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
