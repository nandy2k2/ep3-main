...
import AddUserModal from './Addmpallergies';
import AddUserModalBulk from './Addmpallergiesbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepallergiesbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpallergiesbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpallergiescountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpallergiessecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const duration=user.duration;
            ...
            const response =await ep1.get('/api/v2/updatepallergiesbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
