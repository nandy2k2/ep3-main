...
import { Button, Box, Paper, Container, Grid,  Select, MenuItem, InputLabel } from '@mui/material';
import * as XLSX from 'xlsx';
import { saveAs } from 'file-saver';
import AddUserModal from './Addmmtradingaccount';
import AddUserModalBulk from './Addmmtradingaccountbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtradingaccountbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      ...
      const response = await ep1.get('/api/v2/populatetradingaccount', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtradingaccountcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtradingaccountsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const accgroup=user.accgroup;
const account=user.account;
const acctype=user.acctype;
const cogs=user.cogs;
const amount=user.amount;
const debit=user.debit;
const credit=user.credit;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatemtradingaccountbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
    ...
    return (
      <React.Fragment>
        <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
                        <p>Generate Trading Account, P&L Account and Balance Sheet</p>
            <br /><br />
