...
import AddUserModal from './Addmpconsent';
import AddUserModalBulk from './Addmpconsentbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepconsentbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpconsentbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpconsentcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpconsentsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const consentby=user.consentby;
const relation=user.relation;
const cdate=new Date(user.cdate);
...
const doccomments=user.doccomments;
            ...
            const response =await ep1.get('/api/v2/updatepconsentbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
