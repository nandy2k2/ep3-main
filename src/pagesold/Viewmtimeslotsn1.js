...
import AddUserModal from './Addmtimeslotsn1';
import AddUserModalBulk from './Addmtimeslotsn1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletetimeslotsn1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gettimeslotsn1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gettimeslotsn1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gettimeslotsn1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const slotstatus=user.slotstatus;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatetimeslotsn1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
