...
import AddUserModal from './Addmbtrialb';
import AddUserModalBulk from './Addmbtrialbbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletebtrialbbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getbtrialbbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getbtrialbcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getbtrialbsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const ledgerref=user.ledgerref;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatebtrialbbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
