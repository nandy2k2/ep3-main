...
import AddUserModal from './Addmlmsvideosc';
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/lmsvideoscdocs', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
        const response = await ep1.get('/api/v2/getlmsvideosccount', {
          params: {
            token: token,
            colid: colid,
            user: user
      ...
      const getgraphdatasecond = async () => {
        const response = await ep1.get('/api/v2/getlmsvideoscsecond', {
          params: {
            token: token,
            colid: colid,
            user: user
