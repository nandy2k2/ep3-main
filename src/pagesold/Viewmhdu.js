...
import AddUserModal from './Addmhdu';
import AddUserModalBulk from './Addmhdubulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletehdubyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/gethdubyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gethducountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gethdusecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const capacity=user.capacity;
            ...
            const response =await ep1.get('/api/v2/updatehdubyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
