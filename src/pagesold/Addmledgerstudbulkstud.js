...
return;
  ...
  const searchapi = async (rownumber,academicyear,student,regno,feegroup,semester,feeeitem,feecategory,classdate,amount,paymode,paydetails,installment,status,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createledgerstudstudbyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Fee item' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Fee category' + ',';
return;
}
 if(isNaN(new Date(row[7]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Due date' + ',';
return;
}
...
if(!row[9]){
f1=f1 + 'row ' + rownumber + '-' + 'Pay mode' + ',';
return;
}
if(!row[10]){
f1=f1 + 'row ' + rownumber + '-' + 'Pay details' + ',';
return;
}
if(!row[11]){
f1=f1 + 'row ' + rownumber + '-' + 'Installment' + ',';
return;
}
if(!row[12]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      academicyear,student,regno,feegroup,semester,feeeitem,feecategory,classdate,amount,paymode,paydetails,installment,status,status <br /><br />
