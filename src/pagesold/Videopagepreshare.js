...
const VideoPage = () => {
  ...
  const [videos, setVideos] = useState(videos2);
  ...
  const fetchViewPage = async () => {
    const response = await ep1.get('/api/v2/getlmsvideoscsbyfac', {
      params: {
        //token: token,
        // colid: colid,
        // user: user,
    ...
    setVideos1(response.data.data.classes);
    console.log(videos);
    setCurrentIndex(0);
    //handleSpeakText();
  };
  ...
  useEffect(() => {
    const updateVoices = () => {
      ...
      if (availableVoices.length > 0 && !selectedVoice) {
        ...
        global1.selvoice=availableVoices[0];
      }
    };
  ...
  const showvideos = async() => {
    global1.videos1=videos1;
    global1.selectedVoice=selectedVoice;
    global1.videoid=videoid;
   
    navigate('/videopage31');
  };
