...
import AddUserModal from './Addmlmsvideosc3';
import AddUserModalBulk from './Addmlmsvideoscbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletelmsvideoscbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getlmsvideoscbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getlmsvideosccountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getlmsvideoscsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const language=user.language;
            ...
            const response =await ep1.get('/api/v2/updatelmsvideoscbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
