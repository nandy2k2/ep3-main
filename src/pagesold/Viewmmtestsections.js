...
import AddUserModal from './Addmmtestsections';
import AddUserModalBulk from './Addmmtestsectionsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemtestsectionsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmtestsectionsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmtestsectionscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmtestsectionssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatemtestsectionsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
