...
import AddUserModal from './Addmpadmhistory';
import AddUserModalBulk from './Addmpadmhistorybulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepadmhistorybyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpadmhistorybyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpadmhistorycountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpadmhistorysecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const doccomments=user.doccomments;
            ...
            const response =await ep1.get('/api/v2/updatepadmhistorybyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
