...
import AddUserModal from './Addmseminar';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteseminarbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'membership',
headerName:'Membership to any organization',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'paper',
headerName:'Name of paper presented if any',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getseminarbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getseminarcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getseminarsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updateseminarbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
