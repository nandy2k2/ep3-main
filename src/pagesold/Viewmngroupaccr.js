...
import AddUserModal from './Addmngroupaccr';
import AddUserModalBulk from './Addmngroupaccrbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletengroupaccrbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
     ...
     const onButtonClickpages = async(e, row) => {
      e.stopPropagation();
      global1.accreditation=row.accreditation;
      global1.qno=row.qno;

      navigate('/dashmnallaccrgroup' );
    };
...
return '';
    ...
    const fetchViewPage = async () => {
      ...
      const response = await ep1.get('/api/v2/getngroupaccrbyfac', {
        params: {
          token: token,
          colid: colid,
          group1:group1
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getngroupaccrcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getngroupaccrsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatengroupaccrbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
