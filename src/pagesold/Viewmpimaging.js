...
import AddUserModal from './Addmpimaging';
import AddUserModalBulk from './Addmpimagingbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepimagingbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
valueFormatter: params => dayjs(params?.value).format('DD/MM/YYYY'),
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getpimagingbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getpimagingcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getpimagingsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const finding=user.finding;
const testdate=new Date(user.testdate);

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatepimagingbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
