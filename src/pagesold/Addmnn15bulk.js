...
return;
  ...
  const searchapi = async (rownumber,program,programcode,year,mooc,moocp,moocper,moocno,type,level,status1) => {
...
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'MOOC provider' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'MOOC percentage' + ',';
return;
}
if(!row[6] || isNaN(row[6])){
f1=f1 + 'row ' + rownumber + '-' + 'No of MOOC courses' + ',';
return;
}
if(!row[7]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      program,programcode,year,mooc,moocp,moocper,moocno,type,level,status <br /><br />
