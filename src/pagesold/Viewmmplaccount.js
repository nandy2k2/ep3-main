...
import AddUserModal from './Addmmplaccount';
import AddUserModalBulk from './Addmmplaccountbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemplaccountbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmplaccountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmplaccountcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmplaccountsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const credit=user.credit;
            ...
            const response =await ep1.get('/api/v2/updatemplaccountbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
