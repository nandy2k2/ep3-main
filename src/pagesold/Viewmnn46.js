...
import AddUserModal from './Addmnn46';
import AddUserModalBulk from './Addmnn46bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn46byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn46byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn46countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn46secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const extaudit=user.extaudit;
const intaudit=user.intaudit;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn46byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
level:level,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.year + '~audit';
