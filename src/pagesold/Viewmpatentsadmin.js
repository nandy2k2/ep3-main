...
import AddUserModal from './Addmpatents';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletepatentsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/patentsdocs', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
        const response = await ep1.get('/api/v2/getpatentscount', {
          params: {
            token: token,
            colid: colid,
            user: user
      ...
      const getgraphdatasecond = async () => {
        const response = await ep1.get('/api/v2/getpatentssecond', {
          params: {
            token: token,
            colid: colid,
            user: user
...
const patentnumber=user.patentnumber;
const doa=new Date(user.doa);
const agency=user.agency;
const yop=user.yop;
const doclink=user.doclink;
const patentstatus=user.patentstatus;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatepatentsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
