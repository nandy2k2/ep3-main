...
import AddUserModal from './Addmmcalendar';
import AddUserModalBulk from './Addmmcalendarbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemcalendarbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmcalendarbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmcalendarcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmcalendarsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: lmsuser,
...
const monthofyea=user.monthofyea;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatemcalendarbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
