...
import AddUserModal from './Addmlmsvideos';
import AddUserModalBulk from './Addmlmsvideosbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletelmsvideosbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
          ...
          { field: 'actions', headerName: 'Actions', width: 430, renderCell: (params) => {
            return (
             <table>
                <tr>
                  <td>
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getlmsvideosbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getlmsvideoscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getlmsvideossecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const target=user.target;
            ...
            const response =await ep1.get('/api/v2/updatelmsvideosbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
target:target,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.name + '~' + row.title + '~' + row.description;
