...
import AddUserModal from './Addmmpurchase';
import AddUserModalBulk from './Addmmpurchasebulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletempurchasebyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'approvaluser',
headerName:'Approved by username',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmpurchasebyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmpurchasecountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmpurchasesecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const ponumber=user.ponumber;
const podate=new Date(user.podate);
...
const potype=user.potype;
const approval=user.approval;
const approvaluser=user.approvaluser;
const doclink=user.doclink;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatempurchasebyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
