...
return;
  ...
  const searchapi = async (rownumber,year,extaudit,intaudit,budget,doclink,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn46byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'External audit' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Internal audit' + ',';
return;
}
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Budget' + ',';
return;
}
if(!row[4]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,extaudit,intaudit,budget,doclink,type,level,status <br /><br />
