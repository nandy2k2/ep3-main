...
import AddUserModal from './Addmmdiscussion';
import AddUserModalBulk from './Addmmdiscussionbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemdiscussionbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
valueFormatter: params => dayjs(params?.value).format('DD/MM/YYYY'),
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmdiscussionbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmdiscussioncountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmdiscussionsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const submitdate=new Date(user.submitdate);
            ...
            const response =await ep1.get('/api/v2/updatemdiscussionbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
