...
import AddUserModal from './Addmmattcalc';
import AddUserModalBulk from './Addmmattcalcbulk';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemattcalcbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmattcalcbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmattcalccountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
...
const marksp=user.marksp;
const weightage=user.weightage;
const percentage= parseFloat(marksp) * parseFloat(weightage);         //user.percentage;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatemattcalcbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
    ...
    return (
      <React.Fragment>
        <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
          Course {coursename} {coursecode} <br /><br />
