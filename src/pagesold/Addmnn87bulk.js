...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,appeared,passed,repeat,doclink,type,level,status1) => {
...
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'Appeared' + ',';
return;
}
if(!row[4] || isNaN(row[4])){
f1=f1 + 'row ' + rownumber + '-' + 'Passed' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'Repeat applied' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,appeared,passed,repeat,doclink,type,level,status <br /><br />
