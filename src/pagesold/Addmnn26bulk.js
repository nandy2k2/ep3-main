...
return;
  ...
  const searchapi = async (rownumber,year,faculties,students,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn26byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
f1=f1 + 'row ' + rownumber + '-' + 'Total no of faculties' + ',';
return;
}
if(!row[2] || isNaN(row[2])){
f1=f1 + 'row ' + rownumber + '-' + 'Total no of students' + ',';
return;
}
if(!row[3]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,faculties,students,type,level,status <br /><br />
