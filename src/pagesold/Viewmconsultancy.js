...
import AddUserModal from './Addmconsultancy';
...
function ViewPage() {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteconsultancybyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'title',
headerName:'Title of consultancy',
type:'text',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getconsultancybyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getconsultancycountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getconsultancysecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const doclink=user.doclink;
            ...
            const response =await ep1.get('/api/v2/updateconsultancybyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
