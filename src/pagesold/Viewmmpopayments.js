...
import AddUserModal from './Addmmpopayments';
import AddUserModalBulk from './Addmmpopaymentsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletempopaymentsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmpopaymentsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmpopaymentscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmpopaymentssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const frombank=user.frombank;
const amount=user.amount;
const invoicedate=new Date(user.invoicedate);
const paydate=new Date(user.paydate);
const fromaccount=user.fromaccount;
const fromifsc=user.fromifsc;
const refno=user.refno;
const tobank=user.tobank;
const toaccount=user.toaccount;
const toifsc=user.toifsc;
const paytype=user.paytype;
const doclink=user.doclink;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatempopaymentsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
