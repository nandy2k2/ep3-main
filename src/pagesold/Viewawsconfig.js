...
import AddUserModal from './Addmtestnew';
import AddUserModalBulk from './Addmtestnewbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletetestnewbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    //   global1.testid=row._id;
      global1.awsconfigid=row._id;
      global1.username=row.username;
      global1.password=row.password;
      global1.region=row.region;
      global1.bucket=row.bucket;
      global1.type2=row.type;

      alert('AWS config set');
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v1/getaws', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gettestnewcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gettestnewsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatetestnewbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
<Box sx={{ marginLeft: 3}}>
    <br /><br />
If no bucket is visible, please send an email to support@campus.technology.
            <br /><br />
</Box>
