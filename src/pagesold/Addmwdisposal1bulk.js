...
return;
  ...
  const searchapi = async (rownumber,location,type,agency,binid,collectiondate,disposaldate,disposaltime,vendor,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createwdisposal1byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
f1=f1 + 'row ' + rownumber + '-' + 'Disposal date' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Disposal time' + ',';
return;
}
if(!row[7]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      location,type,agency,binid,collectiondate,disposaldate,disposaltime,vendor,status <br /><br />
