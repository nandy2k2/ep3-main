...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,requests,resolved,changed,doclink,type,level,status1) => {
...
if(!row[3] || isNaN(row[3])){
f1=f1 + 'row ' + rownumber + '-' + 'No of complaints' + ',';
return;
}
if(!row[4] || isNaN(row[4])){
f1=f1 + 'row ' + rownumber + '-' + 'No of resolutions' + ',';
return;
}
if(!row[5] || isNaN(row[5])){
f1=f1 + 'row ' + rownumber + '-' + 'No of changes' + ',';
return;
}
if(!row[6]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,requests,resolved,changed,doclink,type,level,status <br /><br />
