...
import AddUserModal from './Addmtesto';
import AddUserModalBulk from './Addmtestobulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletetestobyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/gettestocountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/gettestosecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const difficulty=user.difficulty;
            ...
            const response =await ep1.get('/api/v2/updatetestobyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
