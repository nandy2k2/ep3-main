...
import AddUserModal from './Addmptreatment';
import AddUserModalBulk from './Addmptreatmentbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteptreatmentbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
valueFormatter: params => dayjs(params?.value).format('DD/MM/YYYY'),
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getptreatmentbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getptreatmentcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getptreatmentsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const startdate=new Date(user.startdate);
            ...
            const response =await ep1.get('/api/v2/updateptreatmentbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
