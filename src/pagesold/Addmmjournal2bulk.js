...
return;
  ...
  const searchapi = async (rownumber,year,accgroup,account,acctype,transaction,transactionref,subledger,cogs,activitydate,amount,credit,debit,type,student,regno,empid,status1) => {
...
const processfile=async()=> {
    ...
    const res=await readXlsxFile(selectedFile).then((rows) => {
        ...
        rows.forEach((row) => {
            ...
            //rownumber=rownumber + 1;
            if(!row[0]){
f1=f1 + 'row ' + rownumber + '-' + 'Financial year' + ',';
return;
}
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Account group' + ',';
return;
}
if(!row[2]){
f1=f1 + 'row ' + rownumber + '-' + 'Account' + ',';
return;
}
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Account type' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Transaction' + ',';
return;
}
if(!row[5]){
f1=f1 + 'row ' + rownumber + '-' + 'Reference if any' + ',';
return;
}
if(!row[6]){
f1=f1 + 'row ' + rownumber + '-' + 'Sub ledger' + ',';
return;
}
if(!row[7]){
f1=f1 + 'row ' + rownumber + '-' + 'Cost of goods sold' + ',';
return;
}
 if(isNaN(new Date(row[8]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Ativity date' + ',';
return;
}
...
f1=f1 + 'row ' + rownumber + '-' + 'Credit' + ',';
return;
}
if(!row[11] || isNaN(row[11])){
f1=f1 + 'row ' + rownumber + '-' + 'Debit' + ',';
return;
}
if(!row[12]){
...
if(!row[15]){
f1=f1 + 'row ' + rownumber + '-' + 'Employee id' + ',';
return;
}
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,accgroup,account,acctype,transaction,transactionref,subledger,cogs,activitydate,amount,credit,debit,type,student,regno,empid,status <br /><br />
