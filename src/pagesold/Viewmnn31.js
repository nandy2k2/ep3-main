...
import AddUserModal from './Addmnn31';
import AddUserModalBulk from './Addmnn31bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn31byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn31byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn31countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn31secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const Faccode=user.Faccode;
const mlocation=user.mlocation;
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn31byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
