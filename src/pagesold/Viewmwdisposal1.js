...
import AddUserModal from './Addmwdisposal1';
import AddUserModalBulk from './Addmwdisposal1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletewdisposal1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getwdisposal1byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getwdisposal1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getwdisposal1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const collectiondate=user.collectiondate;
const disposaldate=new Date(user.disposaldate);
const disposaltime=user.disposaltime;
const vendor=user.vendor;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updatewdisposal1byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
