...
return;
  ...
  const searchapi = async (rownumber,patient,puser,admdate,admtime,admstatus,doctor,status1) => {
...
if(!row[1]){
f1=f1 + 'row ' + rownumber + '-' + 'Patient user id' + ',';
return;
}
 if(isNaN(new Date(row[2]))) {
f1=f1 + 'row ' + rownumber + '-' + 'Admission date' + ',';
return;
}
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Admission time' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Admission status' + ',';
return;
}
if(!row[5]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      patient,puser,admdate,admtime,admstatus,doctor,status <br /><br />
