...
import AddUserModal from './Addmnn56';
import AddUserModalBulk from './Addmnn56bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn56byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'students',
headerName:'No of students using',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'faculty',
headerName:'No of faculties using',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn56byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn56countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn56secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn56byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
