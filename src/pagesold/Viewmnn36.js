...
import AddUserModal from './Addmnn36';
import AddUserModalBulk from './Addmnn36bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn36byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn36byfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn36countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn36secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn36byfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
...
level:level,
    ...
    const onButtonClickgo = async(e, row) => {
      e.stopPropagation();
      var itemstocheck=row.year + '~' + row.facility + '~' + row.location;
