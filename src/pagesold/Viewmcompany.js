...
import AddUserModal from './Addmcompany';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletecompany', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getallcompanies', {
        params: {
          token: token,
          colid: colid,
          user: user
...
//alert(user.email + ' ' + user._id + ', phone ' + Phone + ', password ' + Password+ ', department ' + Department);
            ...
            const response =await ep1.get('/api/v2/updatecompany', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
