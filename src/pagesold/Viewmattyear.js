...
import AddUserModal from './Addmattyear';
import AddUserModalBulk from './Addmattyearbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteattyearbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'pattained',
headerName:'Percentage attained',
type:'number',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getattyearbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getattyearcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getattyearsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const co=user.co;
const attained=user.attained;
const total=user.total;
const pattained=user.pattained;
const level1=user.level1;
const level2=user.level2;
const level3=user.level3;
const target=user.target;
const fleveln=user.fleveln;
const flevel=user.flevel;
const doclink=user.doclink;

            //alert(coursetitle + ' - ' + studentscompleted);
             
     
            const response =await ep1.get('/api/v2/updateattyearbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
