...
import AddUserModal from './Addmmicu';
import AddUserModalBulk from './Addmmicubulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemicubyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmicubyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmicucountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmicusecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const capacity=user.capacity;
            ...
            const response =await ep1.get('/api/v2/updatemicubyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
