...
import AddUserModal from './Addmminewm';
import AddUserModalBulk from './Addmminewmbulk';
...
//   };
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deleteminewmbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const onButtonClicksessions = async(e, row) => {
          e.stopPropagation();
          global1.internshipid=row._id;
          global1.internship=row.testc;
          global1.icoursecode=row.coursecode;
          
          navigate('/dashmmisessions');
        };
    
        const onButtonClicksec = async(e, row) => {
          e.stopPropagation();
          global1.internshipid=row._id;
          global1.internship=row.testc;
          global1.icoursecode=row.coursecode;
          
          navigate('/dashmmisections1');
        };
...
return '';
              ...
              >
                Delete
                  ...
                  <td width="10px"></td>
                                  ...
                                  <td>
                                  <Button
                                onClick={(e) => onButtonClicksessions(e, params.row)}
                                variant="contained" style={{marginTop: 0}}
                              >
                                Sessions
                                  ...
                                  <td>
                                  <Button
                                onClick={(e) => onButtonClicksec(e, params.row)}
                                variant="contained" style={{marginTop: 0}}
                              >
                                Sections
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getminewmcountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getminewmsecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updateminewmbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
    ...
    return (
      <React.Fragment>
        ...
        <Box display="flex" marginBottom={4} marginTop={2}>
          ...
          <Grid container spacing={3} id="export-section">
