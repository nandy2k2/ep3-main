...
return;
  ...
  const searchapi = async (rownumber,location,ctime,vendor,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createwcolschedulebyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      location,ctime,vendor,status <br /><br />
