...
import AddUserModal from './Addmstudalloc1';
import AddUserModalBulk from './Addmstudalloc1bulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletestudalloc1byfac', {
            params: {
                id: row._id,
                token: token,
                user: user
    ...
    const columns = [
        { field: '_id',  width:320, editable: false, headerName: 'Random ID' },
         {
field:'component',
headerName:'Type',
type:'text',
...
//  },
            ...
            return (
             <table>
                <tr>
                  <td>
                  <Button
                onClick={(e) => onButtonClickopen(e, params.row)}
                variant="contained"
              >
                Open link
              </Button>
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/studalloc1examiner', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getstudalloc1countbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getstudalloc1secondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const year=user.year;
            ...
            const response =await ep1.get('/api/v2/updatestudalloc1exam', {
            params: {
            id: user._id,
            token:token,
            colid: colid,
...
status:status,
  ...
  const onButtonClickopen = async(e, row) => {
      e.stopPropagation();
