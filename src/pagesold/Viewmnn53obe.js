...
import AddUserModal from './Addmnn53obe';
import AddUserModalBulk from './Addmnn53obebulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletenn53obebyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
...
{
field:'co',
headerName:'Is CO mapped and assessed',
type:'dropdown',
width:200,
editable:true,
valueFormatter: (params) => {
...
{
field:'po',
headerName:'Is PO mapped and assessed',
type:'dropdown',
width:200,
editable:true,
valueFormatter: (params) => {
...
return '';
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getnn53obebyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getnn53obecountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getnn53obesecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatenn53obebyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
