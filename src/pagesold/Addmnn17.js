...
const revisionpref=useRef();
const bosdateref=useRef();
...
const year=yearref.current.value;
const revision=revisionref.current.value;
const revisionp=revisionpref.current.value;
const bosdate=bosdateref.current.value;
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn17byfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
<InputLabel id="revision">Is revised</InputLabel><Select labelId="revision"
id="revision"
...
<p>Revision percentage</p>
<TextField id="outlined-basic"  type="number" sx={{ width: "100%"}} label=""  variant="outlined" inputRef={revisionpref} /><br /><br />

<LocalizationProvider dateAdapter={AdapterDayjs}><DatePicker label="BoS Date" inputRef={bosdateref} sx={{ width: "100%"}} /></LocalizationProvider><br /><br />
