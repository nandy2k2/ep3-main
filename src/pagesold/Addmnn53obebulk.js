...
return;
  ...
  const searchapi = async (rownumber,year,program,programcode,co,po,doclink,type,level,status1) => {
...
//alert(coursetitle + '-' + dateadded);
    ...
    //setLoading(true);
    //setIsuploading(true);
    const response = await ep1.get('/api/v2/createnn53obebyfac', {
        params: {
            user: user,
            token: token,
            colid: colid,
...
if(!row[3]){
f1=f1 + 'row ' + rownumber + '-' + 'Is CO mapped and assessed' + ',';
return;
}
if(!row[4]){
f1=f1 + 'row ' + rownumber + '-' + 'Is PO mapped and assessed' + ',';
return;
}
if(!row[5]){
...
const getfiledata=()=> {
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose}>
      ...
      year,program,programcode,co,po,doclink,type,level,status <br /><br />
