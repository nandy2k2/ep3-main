...
import AddUserModal from './Addmmisessions';
import AddUserModalBulk from './Addmmisessionsbulk';
  ...
  const handleImageUpload = (event) => {
    ...
    const handleDeleteClick = async (id) => {
      ...
      const onButtonClick = async(e, row) => {
        ...
        //alert(row._id);
        const response = await ep1.get('/api/v2/deletemisessionsbyfac', {
            params: {
                id: row._id,
                token: token,
                user: user
     ...
     const onButtonClickenrol = async(e, row) => {
          e.stopPropagation();
          global1.isessionid=row._id;
          global1.isession =row.session;
          
          
          navigate('/dashmmiseenrol1');
        };
    
        const onButtonClickscores = async(e, row) => {
          e.stopPropagation();
          global1.isessionid=row._id;
          global1.isession =row.session;
          
          
          navigate('/dashmmiscorenew');
        };
...
return '';
          ...
          { field: 'actions', headerName: 'Actions', width: 330, renderCell: (params) => {
            return (
             <table>
                <tr>
                  <td>
              ...
              >
                Delete
                 ...
                 <td width="10px"></td>
                                  <td>
                                  <Button
                                onClick={(e) => onButtonClickenrol(e, params.row)}
                                variant="contained"
                              >
                                Enroll students
                                  ...
                                  <td>
                                  <Button
                                onClick={(e) => onButtonClickscores(e, params.row)}
                                variant="contained"
                              >
                                Marks
    ...
    const fetchViewPage = async () => {
      const response = await ep1.get('/api/v2/getmisessionsbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user,
    ...
    const getgraphdata = async () => {
      const response = await ep1.get('/api/v2/getmisessionscountbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
    ...
    const getgraphdatasecond = async () => {
      const response = await ep1.get('/api/v2/getmisessionssecondbyfac', {
        params: {
          token: token,
          colid: colid,
          user: user
...
const level=user.level;
            ...
            const response =await ep1.get('/api/v2/updatemisessionsbyfac', {
            params: {
            id: user._id,
            user: user.user,
            token:token,
